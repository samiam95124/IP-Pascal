...
procedure parprcfnci(proc: boolean; ss: tolkset; pp: typptr; var tp: typptr); 
          forward;
...
begin
   ...
   if nxttlk = clparen then begin { (const) }
      ...
      if not (tp^.t in [ticst, trcst, tenme, tscst, tccst, tstcst]) then begin
         ...
         if tp^.t <> tudf then perror(esymtyp, [], []); { wrong type }
      ...
      if nxttlk <> crbrkt then repeat { set elements }
         ...
         if not (tp1^.t in [tudf, tinteger, tenum, tboolean, tchar]) then begin
         ...
         if nxttlk = crange then begin
            ...
            tp1 := baset(tp); { get base type }
            if chart(tp1) then tp1 := gblchr; { convert character type }
            if not (tp1^.t in [tudf, tinteger, tenum, tboolean, tchar]) then
               perror(einvstb, [], []); { not a valid set builder }
            { check types of contructors match }
            ...
            if stcc <> nil then begin
               ...
               if ssgtr(stcc^.stee.s, stcc^.stee.v, 
                        stcc^.stes.s, stcc^.stes.v) then perror(einvsub, [], [])
...
begin
   ...
   chktkmp([carray, crecord]); { check possible tolken misspell }
         ...
         if fl <> nil then { find next }
            ...
            if tp1^.t <> tudf then begin { tag defined, look it up }
               ...
               while fl <> nil do begin { search }
                  ...
                  if (fl^.fcsc.v = tv) and (fl^.fcsc.s = ts) then begin 
...
begin
   ...
   if not (tp^.t in [tudf, tvar, tfix, tpar, tvpar, twpar, tfield, tftag]) then
      begin

      { wrong type of head }
      copy(extlab, sp^.lab^); { place error label. Note that nil case should
                                  not get here }
      perror(evartex, [], []); { variable type expected }
   ...
   if vt^.t = tvpar then begin
      ...
      if tp^.t = tgarry then { general array }
         wrtcod(ilditgp) { load indirect tagged pointer }
   ...
   while nxttlk in [clbrkt, cperiod, ccmf] do begin
      ...
      if nxttlk = clbrkt then begin { array index }
            ...
            if tp^.t = tarray then begin { is an array }
               ...
               if not typcmpa(tp^.arri, tp1) then begin
                  ...
                  if tp1^.t in [ticst, tscst, tccst, tenme] then begin
                     ...
                     if ssgtr(ts, ti,  lbounds(tp^.arri), lbound(tp^.arri)) or 
                        ssgtr(ubounds(tp^.arri), ubound(tp^.arri), ts, ti) then
               ...
               if not intt(tp1) and (tp1^.t <> tudf) then begin
                  ...
                  if tp1^.t = ticst then 
                     if ssgtr(constis(tp1), consti(tp1), false, 1) then
                        perror(earrbnd, [], []); { array reference bounds }
                  wrtcod(iarfgar); { output array offset operator }
                  wrtlnk(tp); { output address of array entry }
         ...
         if (tp^.t <> trecord) and (tp^.t <> tudf) then begin { wrong type }
   
            perror(erectex, [], []); { must be record type }
            tp := gbludf { set result undefined }
         ...
         if tp^.t = trecord then begin { search for record scoped symbol }
...
begin
   ...
   if nxttlk in [cidentifier, cperiod, ccmf, clbrkt] then begin 
...
begin
   ...
   if varref <> nil then begin { a variable reference leader exists }
      ...
      if bt^.t in [tinteger, treal, tsreal, tchar, tboolean, tptr, tsub,
                   tenum, tset] then { perform load }
         if bt^.t = treal then 
         ...
         end else begin { must be ordinal }
      ...
      chktkmp([cnil, cnot]); { check possible tolken misspell }
      if nxttlk = cidentifier then begin
         ...
         { check any possible sign of function, '(', function type, type 
           ...
           used here }
         tp1 := actt(sp^.typ); { get actual type }
         if (nxttlk = clparen) or 
            { check procedure, function or overload }
            ...
            if (tp1^.t <> tfunc) and (tp1^.t <> tpfunc) and not chkovld(tp1) and
               not ((tp1^.t in [tenum, tsub]) and not fansi) then begin
   
               copy(extlab, sp^.lab^); { place error label }
               if fansi then perror(embfunc, [], []) { must be function }
               else perror(embfnty, [], []) { must be function }
            ...
            if (tp1^.t in [tenum, tsub]) and not fansi then begin 
               ...
               if tp^.t = ticst then begin { operand is integer constant }
                  ...
                  { it's constant, we can find the corresponding enumerated
                    value right here }
                  if ssgtr(tp^.ival.s, tp^.ival.v, lbounds(tp1), lbound(tp1)) or
                     ssgtr(ubounds(tp1), ubound(tp1), tp^.ival.s, tp^.ival.v) then
            ...
            end else parprcfnci(false, ss, sp^.typ, tp) { parse function call }
         ...
         { check is simple, or loadable quanta }
         if tp^.t in [tinteger, treal, tsreal, tchar, tboolean, tptr, tsub,
                      tenum, tset] then begin
...
begin
   ...
   case tk of { operator }
      ...
      cin: if lft then m := (tp1^.t in [tinteger, tchar, tboolean, tenum]) or
                            chart(tp1) { left side }
           else m := sett(tp1); { must be set }

      cequ, cnequ, cnequa:
         m := (tp1^.t in [tinteger, tchar, tboolean, tenum, treal, tsreal,
                          tptr, tnil]) or
                        sett(tp1) or chart(tp1) or strt(tp1);

      clequ, clequa, cgequ, cgequa:
         m := (tp1^.t in [tinteger, tchar, tboolean, tenum, treal, tsreal]) or
              sett(tp1) or chart(tp1) or strt(tp1);

      cltn, cgtn: m := (tp1^.t in [tinteger, tchar, tboolean, tenum, treal,
                                   tsreal]) or chart(tp1) or strt(tp1)
...
begin
   ...
   { parse simple expression }
   parsexpr(varref, [cequ, cltn, cgtn, cnequ, clequ, cgequ, cin]+ss, tp);
   ...
   if nxttlk in [cequ, cltn, cgtn, cnequ, cnequa, clequ, clequa, cgequ, cgequa, 
      ...
      gettlk; { next }
      { parse simple expression }
      parsexpr(nil, [cequ, cltn, cgtn, cnequ, clequ, cgequ, cin]+ss, tp1);
      { fetch indirect any string characters }
      ...
      if ((tp^.t in [ticst, tenme, tccst]) or 
         ...
         case tk of { operation }
            ...
            cnequ: if (constis(tp) <> constis(tp1)) or
                      (consti(tp) <> consti(tp1)) then
                      tp := gbltrue else tp := gblfalse;
            clequ: if not ssgtr(constis(tp1), consti(tp1), 
                                constis(tp), consti(tp)) then
                      tp := gbltrue else tp := gblfalse;
            cgequ: if not ssgtr(constis(tp), consti(tp), 
                                constis(tp1), consti(tp1)) then
                      tp := gbltrue else tp := gblfalse;
            cltn:  if ssgtr(constis(tp), consti(tp), 
                            constis(tp1), consti(tp1)) then
                      tp := gbltrue else tp := gblfalse;
            cgtn:  if ssgtr(constis(tp1), consti(tp1), 
                            constis(tp), consti(tp1)) then
                      tp := gbltrue else tp := gblfalse
...
{ find case value in case list }

function fndcasval(cv: integer): boolean;
...
begin
   ...
   while cp <> nil do begin { traverse case value list }
      ...
      if cp^.def and (cp^.val = cv) then f := true; { found }
   ...
   repeat { section }
      ...
      repeat { case label }
         ...
         if tp1^.t in [ticst, tscst, tccst, tenme] then begin
            ...
            if tp1^.t <> tudf then cv := consti(tp1) { get value }
            else cv := 0; { no value }
            { process error if case duplicates another on list }
            if fndcasval(cv) then perror(ecasdup, [], []);
            cp^.val := cv; { place value }
         ...
         if (tp^.t in [ticst, tscst, tccst, tenme]) and 
            ...
            { if the selector matches the case, set case match. Then, the
              ...
              statement }
            if (constis(tp) = constis(tp1)) and (consti(tp) = consti(tp1)) then 
               match := true;
...
begin
   ...
   if nxttlk = cidentifier then begin
      ...
      if tp^.t = tvar then begin { simple variable }
         ...
         if not (bt^.t in [tenum, tinteger, tchar, tboolean, tudf]) then begin
            ...
            if tp^.varp then begin { process subroutine threat }

               copy(extlab, sp^.lab^); { place error label }
               perror(eforvst, [], []) { variable has a subroutine threat }
   ...
   if (tp1^.t in [ticst, tscst, tccst, tenme]) and 
      ...
      if tk = cto then 
         cdel := ssgtr(constis(tp2), consti(tp2), constis(tp1), consti(tp1))
      else if tk = cdownto then 
         cdel := ssgtr(constis(tp1), consti(tp1), constis(tp2), consti(tp2));
...
begin
   ...
   { Opps. With overloading we don't actually know what procedure or function we
     are going to execute until its parameters have been evaluated. We go ahead
     and output the "tip" type, but this is just to keep compatibility with the
     intermediate. In reality, no client uses this, and it can be removed. }
      ...
      repeat
         ...
         if pl = nil then begin { parameter does not exist }
            ...
            if over then begin { we are in an overload }
               ...
               if nxttlk = cidentifier then begin { we have a variable tip }

                  find(sp); { lookup symbol }
                  tp1 := actt(sp^.typ); { index type }
                  ...
                  if (tp1^.t in [tproc, tpproc]) and not chkovld(tp1) then begin
                     ...
                     if sysspc(tp1) then begin
                        ...
                        { this could be a function parameter or the start of an
                          ...
                          exist. }
                        winnow(wtype, tp1); { match for assignment 
                                                      type }
                        { check no function matches }
                        if winnil then begin
                        ...
                        if pl^.t = tpfunc then begin
                           ...
                           { parse with variable with threat and parameter
                             flags }
                           parvarh([ccma, crparen]+exprset+ss, true, true, tp, 
                                   verr);
                     ...
                     if tp1^.t in [tvar, tfix, tpar, tvpar, twpar, tfield, 
                        ...
                        parvarh([ccma, crparen]+exprset+ss, false, false, tp,
                                verr);
            ...
            end else if pl^.t = tpproc then begin { procedure parameter }
...
begin
   ...
   if tp^.t = ticst then begin { resolve constant }
      ...
      if ssgtr(tp^.ival.s, tp^.ival.v, lbounds(gblchr), lbound(gblchr)) or
         ssgtr(ubounds(gblchr), ubound(gblchr), tp^.ival.s, tp^.ival.v) then 
         begin
         ...
         repeat { tags }
            ...
            if (tp^.t = tftag) and (tp1^.t <> tudf) then begin
               ...
               while tp <> nil do begin { traverse }
                  ...
                  if (tp^.fcsc.s = s) and (tp^.fcsc.v = n) then 
...
Parse get/put/reset/rewrite/close/update procedure

******************************************************************************}

procedure pargetputresrewclsupd(ss: tolkset; { skip set }
                             dc: prcfnc); { function dispatch code }
...
begin
   ...
   if fansi and (dc = pfclose) then perror(efncprcs, [], []); { not standard }
   expect(clparen, elpexp, [clparen, cidentifier]+ss, []); { expect '(' }
   tp := gbludf; { set result undefined }
   sp := nil; { set no head symbol }
   { parse variable head }
...
begin
   ...
   if (tp^.t in [tarray, tgarry]) and (tp2^.t in [tarray, tgarry]) then begin
      ...
      if ttp <> ttp2 then perror(embscmp, [], [])
         ...
         if ssgtr(ubounds(tp^.arri), ubound(tp^.arri), t1s, t1i) then
...
begin
   ...
   if tp2^.t = tarray then { check index compatible }
      ...
      if ssgtr(ts, ti, lbounds(tp2^.arri), lbound(tp2^.arri)) or
         ssgtr(ubounds(tp2^.arri), ubound(tp2^.arri), ts, ti) then
         perror(epupbnd, [], []) { array reference bounds }
         ...
         t1s := ssadds(constis(tp1), consti(tp1), ts, ti);
         { check that is greater than the upper bound of array }
         if ssgtr(ubounds(tp2^.arri), ubound(tp2^.arri), t1s, t1i) then
            perror(epupbnd, [], []) { yes, out of bounds index }
...
begin
   ...
   repeat { parameters }
      ...
      if nxttlk = cidentifier then begin { id found }
         ...
         find(sp); { find symbol, which also advances the reference count }
         gettlk { get next }
...
begin
   ...
   case dc of { procedure/function }
      ...
      pfnil:      parprcfnc(proc, ss, pp, tp);   { p/f: general 
                                                        procedure/function }
      ...
      pfget:      pargetputresrewclsupd(ss, dc); { p: get }    
      pfnew:      pardispnew(ss, dc);            { p: new }   
      pfpack:     parpack(ss);                   { p: pack }   
      pfpage:     parpage(ss);                   { p: page }    
      pfput:      pargetputresrewclsupd(ss, dc); { p: put }   
      pfread:     parreadln(ss, dc);             { p: read } 
      pfreadln:   parreadln(ss, dc);             { p: readln }  
      pfreset:    pargetputresrewclsupd(ss, dc); { p: reset }
      pfrewrite:  pargetputresrewclsupd(ss, dc); { p: rewrite } 
      ...
      pfclose:    pargetputresrewclsupd(ss, dc); { p: close }  
      ...
      pfupdate:   pargetputresrewclsupd(ss, dc); { p: close }  
...
begin
   ...
   if (tp1^.t in [ticst, tscst, tccst, tenme]) and frange and 
      ...
      if ssgtr(ts, ti, lbounds(tp), lbound(tp)) or 
         ssgtr(ubounds(tp), ubound(tp), ts, ti) then
...
begin
   ...
   if (sp^.typ^.t <> tlab) and (sp^.typ^.t <> tudf) then begin 
      ...
      copy(nxtlab, sp^.lab^); { place errant label }
      perror(esymtyp, [], []);
      ...
      { Check number of references by gotos match number in goto list. If not,
        ...
        this labe. If the statement level is 1, we don't do this check.
        Level 1 statements can be reached from anywhere, and could be
        from an external block. }
      if stalvl > 1 then 
         if cntgto(sp^.typ) < sp^.typ^.lref then perror(elabrds, [], []);
...
{ find matching function in overload list }
...
begin
   ...
   if nxttlk = cbcms then begin { function or variable assign }
      ...
      over := chkovld(sp^.typ); { check its an overload }
      { check procedure label }
      if (sp^.typ^.t <> tproc) and (sp^.typ^.t <> tpproc) and
         (sp^.typ^.t <> tudf) and not over then begin
      ...
      if sp^.typ^.t <> tudf then { not undefined }
         parprcfnci(true, ss, sp^.typ, tp) { parse procedure call }
   ...
   if nxttlk = cidentifier then begin 
      ...
      if nxttlk = crange then begin { subrange }
         ...
         if (tp1^.t <> tudf) and (tp2^.t <> tudf) then begin { operands valid }
            ...
            { check lower <= upper }
            if ssgtr(tp^.subu.s, tp^.subu.v, tp^.subl.s, tp^.subl.v) then 
               perror(einvsub, [], [])
...
begin
   ...
   while tp <> nil do begin { remove entries from source list }
      ...
      if tl = nil then { destination list is empty }
      ...
      else if ssgtr(tt^.fcsc.s, tt^.fcsc.v, tl^.fcsc.s, tl^.fcsc.v) then 
         { new < dest }
         ...
         while tt1 <> nil do begin
            ...
            if tt1 <> nil then { there is a next entry } 
               if ssgtr(tt^.fcsc.s, tt^.fcsc.v, tt1^.fcsc.s, tt1^.fcsc.v) then
...
procedure chkgap(tp: typptr; sl: boolean; l: integer; su: boolean; u: integer);
...
begin
   ...
   if tp <> nil then begin { not empty list }
      ...
      if (l <> tp^.fcsc.v) or (sl <> tp^.fcsc.s) then 
      ...
      while tp <> nil do begin { traverse list }
         ...
         if (tp^.fcsc.v <> ti) or (tp^.fcsc.s <> ts) then begin 

            { case constants not in order }
            if (tp^.fcsc.v = l) and (tp^.fcsc.s = sl) then 
            ...
            l := tp^.fcsc.v; { place new last constant }
            sl := tp^.fcsc.s;
            tp := tp^.fcsn { next entry }
      ...
      repeat { case fields }
         ...
         repeat { case selectors }
            ...
            if tt^.t <> tudf then begin { if defined, start case entry }
               ...
               tt1^.fcsc.v := consti(tt); { place case constant value }
               tt1^.fcsc.s := constis(tt); { place case constant sign }
...
procedure parfphead(    over: boolean; { declaration is overload }
                        ss:   tolkset; { skip set }
...
begin
      ...
      if hsp <> nil then begin { found symbol }
         ...
         if hsp^.typ^.t = tproc then begin { procedure }
            ...
            { Flag no entry if not forwarded, which will end up as a 
              duplicate. Note overload will not complete forward. }
            if not hsp^.typ^.prcf or over then hsp := nil
            ...
            { Flag no entry if not forwarded, which will end up as a 
              duplicate. Note overload will not complete forward. }
            if not hsp^.typ^.fncf or over then hsp := nil
   ...
   if hsp = nil then begin { no forwarded entry found }
      ...
      if nxttlk = cidentifier then begin { id found }
         ...
         end else begin { process overload }

            hsp := lclsym; { find existing symbol }
   ...
   if (nxttlk in [clparen, cidentifier, cvar, cview, cprocedure, 
      ...
      repeat
         ...
         if (nxttlk = cprocedure) or
            ...
            parfphead(false, ss, sp, tp1); { parse header parameter }
...
procedure parfproc(over: boolean; ss: tolkset);
...
begin
   ...
   parfphead(over, [cscn, cforward]+blockset+ss, sp, tp);
   if over then begin { its an overload }
...
var last:      tolken; { tolken save }
    ...
    over:      boolean; { overload is active }
...
begin
   ...
   if fansi then begin
      ...
      if not (nxttlk in blockset+[cfixed, coverload, cperiod]) then 
         { no follow tolken }
         perror(einvblk, blockset+ss, [clabel, cconst, ctype, cvar, cprocedure, 
                cfunction, cfixed, coverload]);
      conset := decset+[cfixed, coverload] { set continuation }
   ...
   while nxttlk in conset do case nxttlk of { tolken }
            ...
            if sp <> nil then begin { there is a symbol }
               ...
               if not sp^.ddf and (sp^.typ^.t = tddf) then begin 
                  ...
                  if not sp^.dra then begin
                     ...
                     copy(extlab, sp^.lab^); { copy label to error label }
                     perror(edradef, [], []); { duplicate error }
                     sp^.dup := true { set symbol is duplicate }
      ...
      cprocedure, cfunction, coverload: begin { procedure/function }
      
         over := false; { set no overload }
         if nxttlk = coverload then begin { overload specified }

            over := true; { set overload active }
         ...
         parfproc(over, conset+[cbegin, cprivate]+ss);
         if not (nxttlk in [cidentifier]+conset+
                           [cbegin, cprivate, cperiod]) then
            { no follow tolken }
            perror(einvblk, [cidentifier, cequ]+conset+
      ...
      if nxttlk = cidentifier then begin { have found an id }
         ...
         while fl <> nil do begin { search file levels }
            ...
            while sp <> nil do begin { search open files }
               ...
               delpth(tn);
               if compp(tn, fn) then dup := true; { found duplicate }
         ...
         while sp <> nil do begin { traverse }
            ...
            { skip empty name entries, which are dummies used for their buffers
              only }
            if sp^.nam[1] <> ' ' then begin { name not null }
         ...
         if fn[1] = ' ' then perror(efnfn, [], []) 
            ...
            opnsrc(fn, maxint, maxint); { open the uses file }
