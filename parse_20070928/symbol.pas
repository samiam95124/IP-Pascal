...
procedure define(var sp: symptr); { symbol to define }
      ...
      if sp1 <> nil then { outter symbol }
         if sp1^.seq >= curseq then begin
            ...
            { Symbol was referenced in this level, or in an encompassed
              ...
              this procedure. }
            extlab := nxtlab; { copy label to error label }
            perror(erefdec, [], []) { referenced before declaration }

      end;
      plcsym(sp); { place as new symbol }
      if sp1 <> nil then { outter symbol }
         if sp1^.seq >= curseq then begin
      ...
      for i := 1 to symmax do begin { traverse the symbols head }
         ...
         while p <> nil do begin { tranverse chain }
            ...
            if (p^.lvl = level) and (p^.ref = 0) and not p^.exp then begin
               ...
               copy(extlab, p^.lab^); { place symbol name for error }
...
begin
   ...
   case tp^.t of { type }
      ...
      tudf: begin write('undefined'); writeln(output) end;           
      tnil: begin write('nil (pointer value)'); writeln(output) end;
      tlab: begin 
      ...
      tinteger: begin write('integer'); writeln(output) end; 
      tchar: begin write('character'); writeln(output) end;
      tboolean: begin write('boolean'); writeln(output) end;
      treal: begin write('real'); writeln(output) end;
      tsreal: begin write('short real'); writeln(output) end;
      ttext: begin write('text'); writeln(output) end;
      teset: begin write('empty set'); writeln(output) end;
      tddf: begin

         write('delayed definition: ');
         listety(tp^.ddft);
         write(' defined: ');
         writeln(output, tp^.ddfd)

      end;
      tglbl: begin write('global block'); writeln(output) end;
   ...
   while tp <> nil do begin { traverse list }
      ...
      if tp^.t = tddf then begin
         ...
         if not tp^.ddfd and not tp^.ddfe then begin
            ...
            copy(extlab, tp^.ddfs^.lab^); { place label for error }
   ...
   if uselvl = 0 then begin { not in a 'uses' file }
      ...
      while tp <> nil do begin { traverse list }
         ...
         if tp^.t = tvar then if tp^.vars = fshparm then begin
            ...
            copy(extlab, sp^.lab^); { place as error label }
         ...
         tlab: if not tp^.ldef or (tp^.lref = 0) then begin 
            ...
            if s <> nil then begin { there is a symbol }
               ...
               if s^.ref <> 0 then begin 
                  ...
                  copy(extlab, s^.lab^); { place for error }
               ...
               end else if not fref and not tp^.ldef then begin
...
begin
   ...
   for i := 1 to max(sp^.lab^) do
      wrtint(chr2ascii(sp^.lab^[i])) { output label characters }
      ...
      while tp <> nil do begin { traverse list }
         ...
         { otherwise perform output according to type }
            ...
            tscst:    begin { string constant }
               ...
               for si := 1 to max(tp^.sval^) do wrtint(chr2ascii(tp^.sval^[si]))
            ...
            tfcas:   begin { record case constant type }
               ...
               wrtnum(tp^.fcsc.s, tp^.fcsc.v) { output case constant }
