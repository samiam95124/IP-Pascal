...
procedure perror(e: errcod; s: tolkset; c: tolkset); forward;
   ...
   if searchreal then begin { search for real specification }
      ...
      if chkchr = '.' then begin { decimal point }
         ...
         if (chkchr = '.') or (chkchr = ')') then 
            ...
            while (chkchr in ['0'..'9']) and not e do begin { parse digits }
   ...
   while not endlin do begin { process string }
      ...
      if (c = '\\') and not fansi then begin { control sequence }
         ...
         if c in ['$', '&', '%', '0'..'9'] then begin
            ...
            parnum(false); { parse numeric (rejecting reals) }
            if nxtint > 255 then begin

               error(echrrng, false); { range error }
               c := chr(0) { replace with null }

            end else c := ascii2chr(nxtint) { place character }
...
begin
   ...
   i := 0; { clear index }
   if fansi then while chkchr in ['0'..'9', 'a'..'z', 'A'..'Z'] do begin
   ...
   end else while chkchr in ['_', '0'..'9', 'a'..'z', 'A'..'Z'] do begin
...
begin
   ...
   if nxttlk <> cundefined then begin { found }
      ...
      nxtlab[2] := ' ';
      search; { try that }
      if nxttlk = cundefined then error(eivsym, false); { error }
...
begin
   ...
   copy(extlab, deftbl[t]^); { place proper spelling }
   error(emspell, false); { output misspell message }
...
begin
   ...
   if not ferrsup then error(e, false); { output error message }
