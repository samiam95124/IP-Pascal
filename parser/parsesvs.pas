...
procedure paropt(optc: char); forward;
...
procedure wrtstr(view s: string); forward;
procedure wrtstrp(view s: string); forward;
...
procedure prthex(f: byte; w: integer); forward;
...
begin
   ...
   if fllstk <> nil then begin
      ...
      if fllstk^.stk <> nil then begin { there are file levels active }
         ...
         wrtstrp(fllstk^.stk^.nam);

      end else begin { output null filename }

         wrtcod(isetsrc); { output source set command }
         wrtstr('') { output null string }
...
procedure prtecd_french(var      f: text;    { file to output to }
                                 e: errcod;  { error to print }
                        view s, s2: string); { parameter strings }

begin

{ obviously, the french messages need to be converted from english }
   case e of { error code }

      efnfn:        write(f, 'File ''', s:0, ''' not found');
      ...
      eivsym:       write(f, 'Invalid symbol/character ''', s:0, 
                             ''' (skipped)');
      eifil:        begin
                    
                       write(f, 'Invalid file specification');
                       if s[1] <> ' ' then write(f, ' ''', s:0, '''')
                    
                    end;
      eioptp:       write(f, 'Option ''#', s:0, ''' is not valid here');
      eterm:        write(f, '''include'' option must be alone on line ',
                             '(remainer of line ignored)');
      eopt:         begin
                    
                       write(f, 'Invalid option');
                       if len(s) > 0 then write(f, ' ''', s:0, '''')
      ...
      empexp:       write(f, '''program''/''module''/''process''/''monitor''',
                             '/''share'' expected');
      ...
      einvpob:      write(f, '''packed'' must be applied to ''array'' ',
                             '''file'', ''set'', or ''record''');
      edecor:       write(f, '''', s:0, ''' declaration out of order');
      ...
      emspell:      write(f, '''', s:0, ''' assumed to be misspelled ''', s2:0, 
                             '''');
      ...
      einvgln:      write(f, 'Goto label ''', s:0, 
                             ''' must be from 1-9999 in value');
      edupsym:      write(f, 'Duplicate definition of ''', s:0, '''');
      esymnf:       write(f, 'Symbol ''', s:0, ''' not found');
      esymnr:       write(f, 'Symbol ''', s:0, 
                             ''' defined but never referenced');
      esymtyp:      write(f, 'Symbol ''', s:0, 
      ...
      einvsub:      write(f, 'Lower bound of subrange must not exceed ', 
                             'higher bound');
      ...
      elabdef:      write(f, 'Goto label ''', s:0, 
                             ''' has multiple destinations');
      elabndf:      write(f, 'Label ''', s:0, 
                             ''' never given destination');
      elabref:      write(f, 'Label ''', s:0, 
                             ''' never referenced by ''goto''');
      elabblk:      write(f, 'Goto label ''', s:0, 
                             ''' must be defined in same block as declaration');
      evartex:      write(f, '''', s:0, ''' must be a variable type');
      earrtex:      write(f, 'Type of variable reference must be array');
      erectex:      write(f, 'Type of variable reference must be record'); 
      eptrtex:      write(f, 'Type of variable reference must be pointer or ', 
                             'file');
      ensftr:       write(f, 'No such field ''', s:0, ''' this record');
      ...
      evarmbl:      write(f, 'For index variable ''', s:0, 
                             ''' must be local to this block');
      evarext:      write(f, 'For index variable ''', s:0, 
                             ''' cannot be external');
      etmpar:       write(f, 'Procedure/function call has more ', 
                             'parameters than definition');
      etlpar:       write(f, 'Procedure/function call has less ', 
                             'parameters than definition');
      eparcmp:      write(f, 'Procedure/function parameter not ', 
                             'compatible with definition');
      evarcmp:      write(f, 'Procedure/function parameter must be same ', 
                             'type as VAR definition');
      ewrtpar:      write(f, 'Parameter of write/writeln incorrect type ', 
                             'for use with text file');
      efldpar:      write(f, 'Type of field width specification must be ', 
                             'integer');
      efrcpar:      write(f, 'Type of fractional digits specification ', 
                             'must be integer');
      embfunc:      write(f, '''', s:0, ''' must be function');
      embfnty:      write(f, '''', s:0, 
                             ''' must be function or type converter');
      embproc:      write(f, '''', s:0, ''' must be procedure');
      embtext:      write(f, 'Writeln can only output to text type file');
      efldtxt:      write(f, 'Field specification can only be applied ',
                             'to text file output');
      ...
      efaslvl:      write(f, 'Function result assign not allowed to ', 
                             'this function');
      ...
      emnbtxt:      write(f, 'Function/procedure cannot be applied to ',
                             'text file type');
      ecmaexp:      write(f, ''','' expected');
      erdpar:       write(f, 'Parameter of read/readln incorrect type ',
                             'for use with text file');
      ...
      embscmp:      write(f, 'Packed and unpacked arrays must have ',
                             'identical component types');
      eidxcmp:      write(f, 'Starting index not compatible with ',
                             'unpacked array index type');
      embptr:       write(f, 'Parameter must be pointer');
      embrec:       write(f, 'Tagfield definition parameters can only ',
                             'be applied to variant records');
      entagf:       write(f, 'No record variant found to correspond to ',
                             'given tagfield');
      ercvcmp:      write(f, 'Tagfield value not compatible with record ',
                             'variant tag');
      ercvnf:       write(f, 'Tagfield value does not have ',
                             'corresponding variant case');
      ...
      enihdf:       write(f, '''input'' file must appear in header ',
                             'for default input');
      enohdf:       write(f, '''output'' file must appear in header ',
                             'for default output');
      ...
      eapreal:      write(f, 'Fractional digits specification ',
                             'applicable to real type only');
      efwddef:      write(f, 'Inappropriate resolution of forward definition');
      emfwddef:     write(f, 'Pointer forward reference ''', s:0,
                             ''' was not defined in block');
      efwdnptr:     write(f, 'Pointer forward reference ''', s:0, 
                             ''' not defined');
      eslfref:      write(f, 'Symbol ''', s:0, 
                             ''' may not occur within definition of same');
      enfncra:      write(f, 'Function ''', s:0, ''' never given result');
      evarass:      write(f, 'Variable ''', s:0, ''' never assigned/modified');
      eforviu:      write(f, 'Variable ''', s:0, 
                             ''' already in use by ''for''');
      eforvst:      write(f, 'Variable ''', s:0, 
                             ''' threatened by subprocedure or subfunction');
      efmbvar:      write(f, 'Parameter ''', s:0, 
                             ''' with file component must be passed as VAR');
      evarmbr:      write(f, 'Variable access must be a record');
      efwdmat:      write(f, 'Forwarded declaration does not match ',
                             'procedure/function type');
      efwdpar:      write(f, 'Forwarded declaration should not have ',
                             'parameters/result');
      eparrep:      write(f, 'Parameters repeated on forward: ',
                             'last occurance used');
      eresrep:      write(f, 'Result repeated on forwarded function: ',
                             'last occurance used');
      eardfwd:      write(f, 'Procedure/function already forwarded');
      efwdndf:      write(f, 'Forwarded procedure/function ''', s:0, 
                             ''' never defined');
      etagvar:      write(f, 'Tagfield cannot appear as a variable parameter');
      epakvar:      write(f, 'Packed component cannot appear as variable ',
                             'parameter');
      ...
      epupbnd:      write(f, 'Pack/unpack operation specifies elements out of ',
                             'bounds');
      eassrng:      write(f, 'Value out of range for destination type');
      egtolvl:      write(f, '''goto'' references label at deeper statement ',
                             'nesting level');
      egtoenc:      write(f, '''goto'' label is not in enclosing statements');
      elabrlv:      write(f, 'Label ''', s:0, 
                             ''' referenced at lesser statement nesting level');
      elabrds:      write(f, 'Label ''', s:0, 
                             ''' referenced by different nested statement');
      elabext:      write(f, 'Label ''', s:0, ''' referenced by another ',
                             'block, must not be nested');
      ...
      eviewth:      write(f, 'View parameter ''', s:0, 
      ...
      efixth:       write(f, 'Fixed object ''', s:0, ''' threatened');
      estelen:      write(f, 'Structured constant length does not match ',
                             'object type');
      einvcse:      write(f, 'Type contains invalid elements for constant ',
                             'structure');
      ...
      emoduse:      write(f, 'Server module is incorrect type for client module');
      eshrvar:      write(f, 'Variables not allowed in ''share'' module');
      emonptr:      write(f, 'Monitor procedure/function parameter cannot ',
                             'contain pointer');
      emonvar:      write(f, 'Variables may not appear in monitor export ',
                             'section');
      ecstopv:      write(f, 'Overflow in constant operation (result replaced',
                             ' with 0)');
      eintovf:      write(f, 'Integer overflow on operator');
      ehfudf:       write(f, 'Header file ''', s:0, 
                             ''' undefined (assumed to be ''text'')');
      esfnprp:      write(f, 'Cannot use system function/procedure as parameter');
      ...
      efwdondf:     write(f, 'Forwarded overload procedure/function ''',
                              s:0, ''' never defined');
      ...
      erefdec:      write(f, 'Symbol ''', s:0, 
                             ''' referenced before declaration');
      edradef:      write(f, 'Forward type reference ''', s:0, 
                             ''' already defined');
      eupdtxt:      write(f, 'Cannot ''update'' text file');
      earrlen:      write(f, 'Array length must be integer');
      euseovf:      write(f, 'Uses path too long');
      edupsta:      write(f, 'Duplicated ''static'' attribute');
      edupovl:      write(f, 'Duplicated ''overload'' attribute');
      embsema:      write(f, 'Parameter must be a semaphore');
      eexsconexp:   write(f, ''';'' or ''on'' or ''except'' expected');
      embbol:       write(f, 'Parameter must be boolean type');
      edupmod:      write(f, 'Symbol ''', s:0, 
                             ''' exists in multiple ''uses'' modules');
      ejnsnf:       write(f, 'Symbol ''', s:0, ''' not found in joined module ''',
                             s2:0, '''');
      edupqual:     write(f, 'Module ''', s:0, 
                             ''' already exists as a joins or uses module');
      esyslab:      write(f, 'System label (label starting with ''_'') ',
                             'not allowed');
      edupvir:      write(f, 'Duplicated ''virtual'' attribute');
      edupovr:      write(f, 'Duplicated ''override'' attribute');
      eattcon:      write(f, 'Conflicting attributes on procedure or function');
      envirtoo:     write(f, 'No procedure or function exists to override');
      evirmat:      write(f, 'Virtual procedure/function type must match its ', 
                             'overrider');
      evirmbe:      write(f, 'Virtual procedure/function must be in an external ', 
                             'module');
      envirpf:      write(f, 'Original procedure/function is not virtual');
      eovrdup:      write(f, 'Override already exists in this module');
      einhmbpfc:    write(f, '''inherited'' attribute must be used on procedure ', 
                             'or function call');
      einhpfp:      write(f, '''inherited'' attribute cannot be used on ',
                             'procedure or function parameter');
      ecalmbor:     write(f, 'Inherited procedure or function call must be ',
                             'override');
      efwdatt:      write(f, 'Forwarded procedure or function attributes do not ',
                             'match');
      efwdorndf:    write(f, 'Forwarded override procedure/function ''', 
                             s:0, ''' never defined');
      eclsexp:      write(f, 'Class expected');
      eclsnf:       write(f, 'Identifier ''', s:0, ''' not found in class ''',
                             s2:0, '''');
      edircls:      write(f, 'Must access class member via reference');
      etoexp:       write(f, '''to'' expected');
      ercrftex:     write(f, 'Record or reference type expected');
      ensmtc:       write(f, 'No such member ''', s:0, ''' this class');
      enacmem:      write(f, '''', s:0, ''' not a member of any class');
      enmemtc:      write(f, '''', s:0, ''' not a member of this class');
      ememvar:      write(f, '''', s:0, ''' must be member variable');
      eidsexp:      write(f, 'Identifier or ''self'' expected');
      eclsact:      write(f, 'No class is currently active for ''self''',
                             ' reference');
      eclsgbl:      write(f, 'Class must be declared in global module');
      eovlnpf:      write(f, 'Overload must be to procedure or function');
      eovrnpf:      write(f, 'Override must be to procedure or function');
      embpor:       write(f, 'Parameter must be pointer or reference');
      einhuse:      write(f, 'Cannot access inherited method here');
      eovrmod:      write(f, 'Override to module procedure/function must be ',
                             'global');
      eovrscls:     write(f, 'Override method to same class as virtual method');
      eovrcdup:     write(f, 'Override already exists in this class');
      eovrnic:      write(f, 'No override exists in current class');
      evarmbrr:     write(f, 'Variable access must be record or reference');
      esnfim:       write(f, 'Symbol ''', s:0, ''' not found in module ''', 
                             s2:0, '''');
      eunfmql:      write(f, 'Unfinished module qualifier ''', s:0, '''');
      evidsexp:     write(f, 'variable identifier or ''self'' expected');
      earrdpt:      write(f, 'Array depth exceeds actual depth');
      einvtrng:     write(f, 'Invalid variant case label range');
      ersltlvl:     write(f, 'Anonymous function result must be at outter level');
      ersltend:     write(f, 'missing ''end'' after anonymous function result');
      ersltfnc:     write(f, 'Anonymous function result not directly enclosed ', 
                             'in function');
      ersltmix:     write(f, 'Anonymous and named function result assignments ',
                             'were mixed');
      eextinv:      write(f, 'External assembly routines not allowed (use a module)');
      esemmod:      write(f, 'Cannot use semaphore outside of monitor');
      esigwatmon:   write(f, 'Cannot use signal or wait outside of monitor');
      embexp:       write(f, 'Parameter must be exception');
      eexexp:       write(f, '''except'' expected');

      { Add new messages here. Don't forget the alternate language cases. }
      ...
      esflt57, 
      esflt58, 
      esflt59:      write(f, 'System fault #', ord(e)-ord(esflt1)+1:1, 
                             ': Notify Moore/CAD')
...
procedure prtecd_english(var      f: text;    { file to output to }
                                  e: errcod;  { error to print }
                         view s, s2: string); { parameter strings }
...
begin
   ...
   case e of { error code }
      ...
      ecasmat:      write(f, 'No case match for selector');
      etagord:      write(f, 'Tag type must be ordinal');
      elvarexp:     write(f, '''', s:0, '''Must be local variable');
      eshrent:      write(f, 'Share must not have entry section');
      eppsext:      write(f, 'Program, process or share cannot have exit sect');
      euseukn:      write(f, 'Used module type is unknown');
      eprgmat:      write(f, 'Program name does not match filename');
      ...
      efwdorndf:    write(f, 'Forwarded override procedure/function ''', 
                             s:0, ''' never defined');
      eclsexp:      write(f, 'Class expected');
      eclsnf:       write(f, 'Identifier not found in class');
      edircls:      write(f, 'Must access class member via reference');
      etoexp:       write(f, '''to'' expected');
      ercrftex:     write(f, 'Record or reference type expected');
      ensmtc:       write(f, 'No such member ''', s:0, ''' this class');
      ...
      esnfim:       write(f, 'Symbol ''', s:0, ''' not found in module ''', 
                             s2:0, '''');
      eunfmql:      write(f, 'Unfinished module qualifier');
      evidsexp:     write(f, 'variable identifier or ''self'' expected');
      earrdpt:      write(f, 'Array depth exceeds actual depth');
      einvtrng:     write(f, 'Invalid variant case label range');
      ersltlvl:     write(f, 'Anonymous function result must be at outter level');
...
procedure prtecd(var      f: text;    { file to output to }
                          e: errcod;  { error to print }
                 view s, s2: string); { parameter string }

begin

   if langmsg = lang_french then prtecd_french(f, e, s, s2)
   else prtecd_english(f, e, s, s2)
...
procedure lsterr(var      f: text;    { error list file }
                          e: errcod;  { error code }
...
begin
   ...
   if not ferrf or not fsrc or (e = efnfn) or (e = eefns) or 
      ...
      lsterr(errfil, e, a, s, s2) { list to error file }
...
{ default empty string parameters }

overload procedure error(     e: errcod;  { error code }
                              a: boolean; { abort flag }
                         view s: string); { parameter string }
...
????? Had to make fn a var parameter then copy from that. See prcmod in
the main source. String was getting trashed, this is a compiler bug.
...
begin
  ...
  repeat
   ...
   until seof or (chkchr > ' ') { eof or not a space or a control }
...
begin
   ...
   if chkchr = '"' then begin { parse string }
      ...
      while (chkchr <> '"') and not endlin do begin { get string characters }
         ...
         if fi = filmax then error(eifil, true, fn); { overflow }
   ...
   if path then begin { check as path }
      ...
      if not validpath(fn) then error(eifil, true, fn)

   end else begin

      if not validfile(fn) then error(eifil, true, fn)
...
begin
   ...
   skpspcl; { skip spaces }
   while chkchr = optc do begin { parse option }
      ...
      if compp(nxtlab, 'verbose') or
         ...
         if fsrc then error(eioptp, false, nxtlab) { error if in source }
      ...
      end else if compp(nxtlab, 'funcres') or
                  compp(nxtlab, 'fr') then begin

         if fsrc then error(eioptp, false, nxtlab) { error if in source }
         else ffncrs := true; { set function result check mode }

      end else if compp(nxtlab, 'nofuncres') or
                  compp(nxtlab, 'nfr') then begin

         if fsrc then error(eioptp, false, nxtlab) { error if in source }
         else ffncrs := false; { set no function result check mode }
      ...
      end else if compp(nxtlab, 'systemlabel') or
                  compp(nxtlab, 'sl') then begin

         if fsrc then error(eioptp, false, nxtlab) { error if in source }
         else fsyslab := true; { set system label mode }

      end else if compp(nxtlab, 'nosystemlabel') or
                  compp(nxtlab, 'nsl') then begin

         if fsrc then error(eioptp, false, nxtlab) { error if in source }
         else fsyslab := false; { set no system label mode }
      ...
      else if compp(nxtlab, 'englishmessages') then
         langmsg := lang_english { set english messages }
      else if compp(nxtlab, 'frenchmessages') then 
         langmsg := lang_french { set french messages }
      else if compp(nxtlab, 'englishkeys') then
         langkey := lang_english { set english keys }
      else if compp(nxtlab, 'frenchkeys') then
         langkey := lang_french { set french keys }
      else error(eopt, false, nxtlab); { no option found }
      skpspcl { skip spaces }
...
begin
   ...
   paropt(optchr); { parse any options }
...
begin
   ...
   if fintopn then begin { generate output }
      ...
      if n = 0 then begin write(intout, 0); write(intout, 0) end else begin
         ...
         { have to reserve this test for when we have true unsigned values }
         {if n < 0 then error(esflt42, true);}
         if s then t := t + $40; { place sign in tag }
         if usedirect then outnum(n, t) { use direct output method }
         else begin
...
Output string to intermediate file

Outputs the given string to the intermediate file. The format is:

1. Length of string, byte.
2. String characters.

This places a 255 character limit on output strings.

*******************************************************************************}

procedure wrtstr(view s: string);

var l: 0..filmax; { length of string }
    i: filinx;    { index for string }

begin

   l := max(s); { find length of string }
   wrtint(l); { output string length }
   { output string characters }
   for i := 1 to l do begin

      wrtint(0); { extend to UCS-16 (Unicode) }
      wrtint(chr2ascii(s[i])) { output ASCII base code }
...
Output padded string to intermediate file

Outputs the given string to the intermediate file. The string is a padded right,
and only the non-padding characters are output. The format is:
...
procedure wrtstrp(view s: string);
