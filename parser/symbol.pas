...
*                             9/89 S. A. Moore                                 *
*                                                                              *
* Handles construction of the symbol table, as well as its output to the       *
* intermediate file. Also handles the type structures, their construction,     *
* output to the intermediate, and several parser support functions that have   *
* to do with type structures. In this parser, virtually all program constructs *
* are build with type entries.                                                 *
*                                                                              *
*******************************************************************************}
...
function mgblsym(view l: string): symptr; forward;
function lclsym(view l: string): symptr; forward;
procedure plcsym(view  l: string; var sp: symptr); forward;
procedure plcsyms(sl: symptr); forward;
procedure purge; forward;
procedure listsym(sp: symptr); forward;
procedure numlab(il: integer; var l: string); forward;
procedure define(view l: string; var sp: symptr); forward;
procedure find(view l: string; var sp: symptr); forward;
...
procedure pshblk(sp: symptr; tp: typptr); forward;
...
function lbounds(tp: typptr): boolean; forward;
function ubound(tp: typptr): integer; forward;
function ubounds(tp: typptr): boolean; forward;
...
procedure chktkmp(view l: string; c: tolkset); forward;
...
procedure getgto(var gp: gtoptr); forward;
procedure putgto(gp: gtoptr); forward;
procedure putgtos(gl: gtoptr); forward;
procedure mrggto(var dl: gtoptr; sl: gtoptr); forward;
...
function sysspc(tp: typptr): boolean; forward;
procedure getwin(var wp: winptr); forward;
procedure putwin(wp: winptr); forward;
procedure putwins(wl: winptr); forward;
procedure chgtyp(s, d: typptr); forward;
procedure reptyp(s, d: typptr); forward;
procedure gettyp(var tp: typptr; t:  types); forward;
procedure dmpblk; forward;
procedure getmod(var mp: modptr); forward;
function fndmod(view mn: string): modptr; forward;
procedure chkconflt; forward;
procedure getmlt(var mp: mltptr); forward;
procedure putmlt(mp: mltptr); forward;
procedure dmpmod; forward;
procedure dmpuse; forward;
procedure dmpjoin; forward;
procedure dmpsymmod(mp: modptr); forward;
procedure dmpsymuse; forward;
function gblsym(view l: string): symptr; forward;
function fndqual(view mn: string): mltptr; forward;
procedure listsyms(sp: symptr); forward;
procedure symudf(view  l: string; var sp: symptr); forward;
function fndmodsym(mp: modptr; view l:  string): symptr; forward;
function clsbas(tp: typptr; view l:  string): symptr; forward;
function chkbas(tp: typptr; cp: typptr): boolean; forward;
function chkmtd(tp: typptr): boolean; forward;
function ovrcnt(tp: typptr): integer; forward;
function ovrfwd(tp: typptr): typptr; forward;
function ovrcntcls(tp: typptr; cls: typptr): integer; forward;
procedure pshbas(tp: typptr); forward;
procedure getwth(var wp: wthptr); forward;
procedure putwth(wp: wthptr); forward;
function chkwth(tp: typptr): boolean; forward;
procedure newsym(view l: string; sp: symptr); forward;
function classt(tp: typptr): boolean; forward;
function reft(tp: typptr): boolean; forward;

private

procedure listety(tp: typptr); forward;
procedure puttyp(tp: typptr); forward;

{*******************************************************************************

Get a new module entry

Gets a new module entry, and initalizes it. There is no module recycling.

*******************************************************************************}

procedure getmod(var mp: modptr);

var i: syminx; { index for symbols hash table }

begin

   new(mp); { get a new module }
   mp^.next := nil; { clear links }
   mp^.inx := 0; { clear module number }
   mp^.modn := nil; { clear name string }
   mp^.modf := nil; { clear file path }
   for i := 1 to symmax do mp^.symtbl[i] := nil { clear symbol hash table }

end;

{*******************************************************************************

Dump the modules list

A diagnostic, dumps the contents of the global module tracking list.
...
begin
   ...
   writeln('Module list:');
   writeln;
   mp := modlst; { index top of modules list }
   while mp <> nil do begin { traverse }

      c := 0; { clear symbol head count }
      { count symbol heads }
      for i := 1 to symmax do if mp^.symtbl[i] <> nil then c := c+1;
      writeln('Module name:           ', mp^.modn^:0);
      writeln('File:                  ', mp^.modf^:0);
      writeln('Module logical number: ', mp^.inx:1);
      writeln('Symbol head count:     ', c:1);
      writeln;
      mp := mp^.next { link next entry }
...
Dumps the indicated modules stack. This is a diagnostic.

*******************************************************************************}

procedure dmpmodstk(mp: mltptr);

begin

   while mp <> nil do begin { traverse }

      writeln('Module name:           ', mp^.modp^.modn^:0);
      mp := mp^.next { link next entry }
...
Dumps the uses stack. This is a diagnostic.
...
Dumps the joins stack. This is a diagnostic.
...
Dump symbols in module

Dumps symbols in the indicated module. A diagnostic.

*******************************************************************************}

procedure dmpsymmod(mp: modptr);
...
begin
   ...
   for i := 1 to symmax do begin { traverse }

      p := mp^.symtbl[i]; { index top of list }
...
Dump symbols in uses list

Dumps symbols in the uses list by module. A diagnostic.
...
begin
   ...
   writeln;
   writeln('Uses list by block:');
   writeln;
   mp := uselst; { index top of uses list }
   while mp <> nil do begin { traverse }

      writeln;
      writeln('Module: ', mp^.modp^.modn^:0);
      writeln;
      dmpsymmod(mp^.modp);
      mp := mp^.next { next list entry }
...
Get a module list entry

Gets a new module list entry, and initalizes it. If one is available on the free
list, that is used, otherwise, a new one is created.

*******************************************************************************}

procedure getmlt(var mp: mltptr);

begin

   if mltfre <> nil then begin { return existing entry }
   
      mp := mltfre; { index top entry }
      mltfre := mltfre^.next { gap the list }

   end else new(mp); { get an entirely new entry }
   mp^.next := nil; { clear next }
   mp^.modp := nil { clear module }

end;

{*******************************************************************************

Put module list entry

Places a used module list entry on the free list to await later reuse.

*******************************************************************************}

procedure putmlt(mp: mltptr);

begin

   mp^.next := mltfre; { link to head }
   mltfre := mp { and place new head }
...
Finds a module by name in the module list. Returns the module pointer if found,
otherwise nil.

*******************************************************************************}

function fndmod(view mn: string): modptr;

var fp: modptr; { found module pointer }
    mp: modptr; { module pointer }

begin

   mp := modlst; { index top of module list }
   fp := nil; { set no module found }
   while mp <> nil do begin { traverse }

      if comp(mp^.modn, mn) then begin { found }

         fp := mp; { set found pointer }
         mp := nil { stop search }

      end else mp := mp^.next { next in module list }

   end;

   fndmod := fp { return found pointer }

end;

{*******************************************************************************

Find qualident module

Finds a module by name in the current joins or uses list. Returns the module
pointer if found, otherwise nil.

*******************************************************************************}

function fndqual(view mn: string): mltptr;

var fp: mltptr; { found module pointer }

{ find in list }

function fndlst(mp: mltptr): mltptr;
...
begin
   ...
   while mp <> nil do begin { traverse }
      ...
      if comp(mp^.modp^.modn, mn) then begin { found }
   ...
   fndlst := fp { return found pointer }

end;

begin

   fp := fndlst(joinlst); { try joins list }
   if fp = nil then fp := fndlst(uselst); { try uses list }
...
Find first symbol table entry

Given a symbol table pointer, and a module entry pointer, and the hash index,
finds the first symbol entry in the table, crossing module boundaries.

If the first module given has a valid symbol at the top, that is returned.
Otherwise, we skip modules until we find one with a defined first entry.

Returns both the symbol pointer and the module pointer nil if no first entry
exists.

*******************************************************************************}

procedure fstsym(var sp: symptr;  { symbol pointer }
...
begin
   ...
   if mp <> nil then begin
      ...
      repeat
         ...
         sp := mp^.modp^.symtbl[i]; { index top entry } 
         { if that block has a null list, move to the next if possible }
         if (sp = nil) and (mp <> nil) then mp := mp^.next

      until (sp <> nil) or (mp = nil) { found entry or end of blocks }
...
Find next symbol table entry

Given a symbol table pointer, and a module entry pointer, and the hash index,
finds the next symbol entry in the table, crossing module boundaries.

If the next symbol pointer is nil, then the next module is indexed, using the
hash index to find the same list in the next module. If the module pointer is
also nil, then the result will be a nil symbol pointer, indicating the the
end of all tables.

*******************************************************************************}

procedure nxtsym(var sp: symptr;  { symbol pointer }
                 var mp: mltptr;  { module pointer }
                     i:  syminx); { hash index }

begin

   sp := sp^.next; { index next symbol entry }
   while (sp = nil) and (mp <> nil) do begin 

      { no symbol, and more blocks exist }
      mp := mp^.next; { index next uses block }
      { if there is a next uses block, just keep going into that }
      if mp <> nil then sp := mp^.modp^.symtbl[i]
...
Find symbol conflicts across modules

Finds if symbol conflicts exist across modules. Searches the entire sybols
pool for the current level, and finds any duplicates. If found, an error 
message indicating the current module is output.

*******************************************************************************}

procedure chkconflt;

var p:   symptr;  { pointers for symbol table }
    i:   integer; { index for symbol table }
    mp:  mltptr;  { module list entry pointer }
    err: boolean; { error flag }

{ search sublist for symbol }

function schlst(view s:  string; { symbol to search for }
                     p:  symptr; { pointer to start of list }
                     mp: mltptr; { module }
                     i:  syminx) { hash index }
                 : boolean;       { found/not found flag }
...
begin
   ...
   { find first entry }
   while (p = nil) and (mp <> nil) do begin

      mp := mp^.next; { next module }
      if mp <> nil then p := mp^.modp^.symtbl[i] { get top symbol }

   end;
   { find symbol }
   while (p <> nil) and not m do begin { search list }

      if comp(s, p^.lab^) then m := true { found match }
      else nxtsym(p, mp, i) { next symbol }

   end;

   schlst := m { return match status }

end;

begin

   i := 1; { set first symbol hash slot }
   err := false; { set no error occurred }
   repeat { traverse the symbols heads }

      mp := uselst; { index top of uses list }
      fstsym(p, mp, i); { find first symbol }
      while p <> nil do begin { traverse chain }

         if schlst(p^.lab^, p^.next, mp, i) then begin { duplicate found }

            perror(edupmod, [], [], p^.lab^); { duplicate }
            err := true { flag error }

         end;
         nxtsym(p, mp, i) { index next entry }

      end;
      i := i+1 { next hash chain }

   until (i > symmax) or err { searched entire table or error }
...
Gets a new winnow entry, either from the free list, or from new store.

*******************************************************************************}

procedure getwin(var wp: winptr);

begin

   if (winfre <> nil) and frecir then begin { return existing symbol entry }
   
      wp := winfre; { index top entry }
      winfre := winfre^.next; { gap the list }
      winfct := winfct - 1 { remove free symbol }

   end else begin 

      new(wp); { get an entirely new entry }
      wincct := wincct + 1 { and count it }

   end;
   wp^.next := nil; { clear next }
   wp^.prcfnc := nil; { clear procedure/function entry }
   winact := winact+1 { count active winnow entries }
...
Search a module for a given symbol. Expects the module to be searched. Returns
either the matching symbol, or nil if not found.

*******************************************************************************}

function fndmodsym(     mp: modptr; { module to search }
                   view l:  string) { label to search for }
                   : symptr;        { symbol or nil }

var p, r: symptr; { pointers for symbol table }
    i:    syminx; { index for symbol tables }

begin

   i := hash(l, 0, symmax); { get hash for symbol }
   p := mp^.symtbl[i]; { index top of hash chain }
   r := nil; { set no symbol found }
   while p <> nil do begin { traverse chain }
   
      if compp(l, p^.lab^) then begin { entry found }
   
         r := p; { place result pointer }
         p := nil { nix search pointer }
   
      end else p := p^.next { entry not found }
   
   end;

   fndmodsym := r { return result }
...
Places a winnow entry to the free list.

*******************************************************************************}

procedure putwin(wp: winptr);

begin

   wp^.next := winfre; { link to head }
   winfre := wp; { and place new head }
   winfct := winfct + 1; { count free winbols }
   winact := winact - 1 { and remove an active winbol }

end;

{*******************************************************************************

Put winnow entry list

Frees a list of winnows.

*******************************************************************************}

procedure putwins(wl: winptr);

var wp: winptr; { winnow pointer }

begin

   while wl <> nil do begin { empty winnows list }

      wp := wl; { index top entry }
      wl := wl^.next; { gap out top entry }
      putwin(wp) { free that }
...
begin
   ...
   cp^.svals := false; { clear start and end values }
   cp^.sval := 0;
   cp^.evals := false;
   cp^.eval := 0;
...
Get a goto tracking entry

Gets a new goto tracking entry, either from the free list, or from new store.

*******************************************************************************}

procedure getgto(var gp: gtoptr);

begin

   if (gtofre <> nil) and frecir then begin { return existing symbol entry }
   
      gp := gtofre; { index top entry }
      gtofre := gtofre^.next; { gap the list }
      gtofct := gtofct - 1 { remove free symbol }

   end else begin 

      new(gp); { get an entirely new entry }
      gtocct := gtocct + 1 { and count it }

   end;
   gp^.next := nil; { clear next }
   gp^.lab := nil; { clear label link }
   gtoact := gtoact+1 { count active case entries }
...
Places a goto tracking entry to the free list.

*******************************************************************************}

procedure putgto(gp: gtoptr);

begin

   gp^.next := gtofre; { link to head }
   gtofre := gp; { and place new head }
   gtofct := gtofct + 1; { count free casbols }
   gtoact := gtoact - 1 { and remove an active casbol }
...
Frees a list of goto tracking entries.

*******************************************************************************}

procedure putgtos(gl: gtoptr);

var gp: gtoptr; { case value pointer }

begin

   while gl <> nil do begin { empty case values list }

      gp := gl; { index top entry }
      gl := gl^.next; { gap out top entry }
      putgto(gp) { free that }
...
Merges two goto lists into one.

*******************************************************************************}

procedure mrggto(var dl: gtoptr; sl: gtoptr);
...
begin
   ...
   if sl <> nil then begin { source list not empty }

      ep := sl; { find end of source list }
      while ep^.next <> nil do ep := ep^.next; { find end }
      ep^.next := dl; { link last entry to start of destination }
      dl := sl { and join the lists }
...
begin
   ...
   sp^.seq  := 0;       { clear sequence of reference }
   ...
   sp^.dra  := false;   { set not a downreference alias }
   sp^.modp := nil;     { clear owning module link }
   sp^.prv  := false;   { symbol is public }
...
Get a 'with' list entry

Gets a new 'with' list entry, and initalizes it. If one is available on the free
list, that is used, otherwise, a new one is created.

*******************************************************************************}

procedure getwth(var wp: wthptr);

begin

   if wthfre <> nil then begin { return existing entry }
   
      wp := wthfre; { index top entry }
      wthfre := wthfre^.next { gap the list }

   end else new(wp); { get an entirely new entry }
   wp^.next := nil; { clear next }
   wp^.varp := nil { clear variable }
...
procedure putwth(wp: wthptr);

begin

   wp^.next := wthfre; { link to head }
   wthfre := wp { and place new head }
...
Checks if the given member exists in the given class, or one of its base 
classes. Returns true if so.

*******************************************************************************}

function chkbas(tp: typptr; { member to check }
                cp: typptr) { class to check }
...
begin
   ...
   b := false; { set does not exist }
   while cp <> nil do begin

      if cp = tp^.classt then b := true; { found in this class }
      { link base class }
      case cp^.t of { class }
      
         tclass:  cp := cp^.clsi; { class }
         tatom:   cp := cp^.atmi; { atom }
         tthread: cp := cp^.thdi; { class }
         
      end

   end;

   chkbas := b { return result }

end;

{*******************************************************************************

Check member exists in 'with' stack

Checks if the given member exists anywhere in the 'with' stack. This is used to
validate an unqualified member access in nested 'with' statements. This function
is only used for classes.

*******************************************************************************}

function chkwth(tp: typptr) { member to check }
                : boolean;  { result status }
    
var wp: wthptr;  { pointer for 'with' entries }
    f:  boolean; { found flag }

begin

   f := false; { set not found }
   wp := wthlst; { index top of 'with' stack }
   while wp <> nil do begin { search for 'with' classes }

      if wp^.varp^.t = treference then { it is an object reference }
         if chkbas(tp, wp^.varp^.reft) then begin { found }

         f := true; { set found }
         wp := nil { terminate search }

      end;
      if wp <> nil then wp := wp^.next { next entry in stack }   

   end;

   chkwth := f { return found flag }
...
Checks if the given type is a method, that is, if it is a procedure or function,
and has the method flag set. Returns true if so, otherwise false. Methods are
specifically procedures or functions that appear in a class. A procedure or
function that is nested within a method is NOT itself a method, rather it is a
subroutine that is called by the method itself. The difference is that it is not
passed a copy of the object, although it may access it from the surrounding
method.

*******************************************************************************}

function chkmtd(tp: typptr): boolean;

var m: boolean; { method flag }

begin

   m := false; { set not a method }
   if tp^.t = tproc then m := tp^.prcm; { get procedure method flag }
   if tp^.t = tfunc then m := tp^.fncm; { get function method flag }

   chkmtd := m { return result }
...
Finds a symbol that matches the given label regardless of the block it belongs
to. Returns a pointer to the matching symbol entry, or nil if none is found.

Downreference markers are ignored, since they are not returned for any
search.

*******************************************************************************}

function gblsym(view l: string) { label to find }
                  : symptr;       { symbol found or nil }

var p, r:      symptr;  { pointers for symbol table }
    mp:        mltptr;  { module entry pointer }
    i:         syminx;  { index for symbol tables }
...
begin
   ...
   if mp <> nil then begin { there are blocks (should never be nil) }
      ...
      while p <> nil do begin { traverse chain }
         ...
         end else nxtsym(p, mp, i) { entry not found }
...
Find symbol in class or base class

Finds if the given identifier exists in the given class or one of its base
classes. Returns the symbol if so.

*******************************************************************************}

function clsbas(tp: typptr; view l:  string): symptr;

var sp: symptr;

begin

   sp := nil; { set no symbol found }
   while (tp <> nil) and (sp = nil) do begin

      case tp^.t of { class }
      
         tclass:  sp := fndmodsym(tp^.clss, l); { class }
         tatom:   sp := fndmodsym(tp^.atms, l); { atom }
         tthread: sp := fndmodsym(tp^.thds, l); { class }
         
      end;
      if sp = nil then
         { link base class (if any) }
         case tp^.t of { class }
         
            tclass:  tp := tp^.clsi; { class }
            tatom:   tp := tp^.atmi; { atom }
            tthread: tp := tp^.thdi; { class }
            
         end

   end;

   clsbas := sp { return symbol }
...
Finds a symbol that matches the given symbol regardless of the block it belongs
to. Returns a pointer to the matching symbol entry, or nil if none is found.
...
function mgblsym(view l: string) { label to find }
                 : symptr;       { symbol or nil }
...
begin
   ...
   r := nil; { clear result pointer }
   for i := 1 to symmax do begin { traverse the symbols heads }

      mp := uselst; { index top of uses list }
      fstsym(p, mp, i); { find first symbol }
      while p <> nil do begin { traverse chain }

         if match(l, p^.lab^) then r := p; { entry found }
         nxtsym(p, mp, i) { index next entry }
...
function lclsym(view l: string) { label }
                : symptr;       { symbol or nil }
...
begin
   ...
   r := gblsym(l); { find matching symbol }
...
procedure plcsym(view  l: string;  { label to place }
                 var  sp: symptr); { returns the symbol entry }
...
begin
   ...
   sp^.modp := modlst; { set owning module }
   sp^.prv := privat; { set private/public }
   new(sp^.lab, len(l)); { get a label string }
   copy(sp^.lab^, l); { place the label }
   i := hash(l, 0, symmax); { find the top entry }
   sp^.next := uselst^.modp^.symtbl[i]; { place the next entry link }
   uselst^.modp^.symtbl[i] := sp { plant our symbol }
...
Replaces a type in the current type list with a new type. The new type replaces
the old type entry in the list, including its sequence number.

Has a check for type below the update bar. If the update has passed the
replaced type, then its already in the intermediate file, and its a serious
error. This routine should not be called when an wrttyp call has happened
since the type was created.

*******************************************************************************}

procedure reptyp(s, d: typptr);

var tp:  typptr; { type list pointer }
    lp:  typptr; { last entry }
    upd: boolean; { past the update bar }

begin

   tp := blkstk^.typ; { index top of list }
   lp := nil; { clear last entry }
   upd := false; { set no update bar found }
   { find entry, last entry pointer }
   while (tp <> nil) and (tp <> s) do begin { traverse }

      if tp = blkstk^.upd then upd := true; { set we passed the update bar }
      lp := tp; { set last }
      tp := tp^.next { link next }

   end;
   if tp = blkstk^.upd then upd := true; { set we passed the update bar }
   if tp = nil then error(esflt38, true); { should have been found }
   if not upd then error(esflt39, true); { we didn't pass update bar }
   if lp = nil then blkstk^.typ := d { place at list top }
   else lp^.next := d; { place mid list }
   d^.next := s^.next; { link to next }
   if blkstk^.upd = s then blkstk^.upd := d; { change update pointer if needed }
   if blkstk^.lst = s then blkstk^.lst := d; { change last pointer if needed }
   d^.lvl := s^.lvl; { copy level }
   d^.num := s^.num; { copy sequential type number }
   puttyp(s) { release source entry }
...
Form rip symbols list copy

Forms a list of the current scope level symbols that are copies of the original
symbols. This is required because the list can be alphabetized. It must be
disposed of after use.

Note that the labels are not copied, just the pointers.

Does not cross module boundaries.

*******************************************************************************}

procedure formlistcopy(var sp: symptr);

var i:     syminx; { index for symbol table }
    p, p2: symptr; { pointers for symbols }
...
begin
   ...
   for i := 1 to symmax do begin { traverse the symbols head }
      ...
      while p <> nil do { traverse }
         ...
         getsym(p2); { get a new symbol entry }
         p2^ := p^; { copy contents of symbol entry }
         p2^.next := sp; { push onto copy list }
         sp := p2;
...
LIsts the indicated symbols entry.

*******************************************************************************}

procedure listsymety(p: symptr);

var li: labinx; { index for labels }

begin

   if fsym then begin { list symbols }

      if not p^.dra then begin { print label }

         { print label with trailing spaces }
         for li := 1 to lsmmax do 
            if li > max(p^.lab^) then write(' ')
            else write(p^.lab^[li]);
         write(' '); { separate }
         write(p^.ref:4); { output reference count }
         write(' '); { separate }
         { print duplicate status }
         if p^.dup then write('duplicate '); 
         { print misspell status }
         if p^.mis then write('misspell '); 
         { print missing definition status }
         if p^.udf then write('undefined ');
         { print exportable status }
         if p^.exp then write('exportable '); 
         if p^.typ = nil then write('<nil>') { output nil for type }
         else listety(p^.typ); { print link }
         writeln { next line }
...
Downreference markers are ignored for the listing.

*******************************************************************************}

procedure listsyms(sp: symptr);
...
begin
   ...
   if fsym then begin { list symbols }
      ...
      formlistcopy(p); { create symbols list from scope }
      alpsym(p); { alphabetize the list }
      writeln; { space off }
      { output header }
      write('Name:  '); 
      if sp <> nil then write(sp^.lab^:0) 
                   else write('<missing>');
      writeln;
      write('Depth: ');
      writeln(output, level:1);
      writeln; { space off }
      while p <> nil do begin { print and dispose of labels }
   
         if not p^.dra then { print label }
            listsymety(p); { list symbol }
         d := p; { save symbol for disposal }
         p := p^.next; { index next symbol }
         { when the copy was made, the symbol string was brought over, not 
           copied. Because of this we can't allow that to be released, so knock
           it out. }
         d^.lab := nil; { remove symbol name }
...
Downreference markers are ignored for the listing, but are removed and disposed
of.
...
Creates a label in the given buffer based on the scanner label nxtint.
...
procedure numlab(    il: integer; { integer version of label }
                 var l:  string); { string result }
...
begin
   ...
   for i := 1 to max(l) do l[i] := ' '; { clear the result buffer }
   i := 1; { set the 1st digit }
   {p := digmax;} { index the maximum digit }
   p := decpow; { index the maximum digit }
   n := il; { get the number to process }
   while p <> 0 do begin { process power }

      l[i] := chr((n div p) + ord('0')); { place digit }
      { move to the next digit, but only if not leading zero }
      if (i > 1) or (l[i] <> '0') then i := i+1;
...
Uses Sale's method for rejecting references before declaration in a block. The
sequence number of each scope is kept, and each reference stores that in the
symbol. This routine checks if any references exist at this, or later
sequences, which would be an invalid reference.

*******************************************************************************}

procedure define(view l:  string;  { symbol label }
                 var  sp: symptr); { symbol to define }

var sp1: symptr; { symbol pointer }

begin

   sp := lclsym(l); { find previous symbol }
   if sp <> nil then begin { duplicate symbol }

      if sp^.ddf then perror(efwddef, [], []) { bad forward definition }
      else if sp^.hld then perror(eslfref, [], [], l) { self reference }
      else perror(edupsym, [], [], l); { symbol is duplicate }
      ...
      { check if any other level has a symbol }
      sp1 := gblsym(l);
      if sp1 <> nil then { outter symbol }
         if sp1^.seq >= curseq then
            { Symbol was referenced in this level, or in an encompassed
              level. Note that pointers follow different rules and don't use
              this procedure. }
            perror(erefdec, [], [], l); { referenced before declaration }
      plcsym(l, sp); { place as new symbol }
      if sp1 <> nil then { outter symbol }
         if sp1^.seq >= curseq then begin

            { because the symbol was already referenced, we add that to the
              brand new symbol's reference count. This suppresses the double
              error of prereference followed by lack of further use. }
            sp^.ref := sp^.ref+1
...
Process symbol not found

Processes the case of the given identifier not found. First, we attempt to find
the symbol as a misspelling. If that is not sucessful, the symbol is created and
given the type undefined. This will hopefully supress further errors.

Returns the new symbol.

*******************************************************************************}

procedure symudf(view  l: string;  { label to respell }
                 var  sp: symptr); { resulting symbol }

var sp1: symptr; { symbol pointer }

begin

   sp1 := mgblsym(l); { find any misspelled global symbol }
   { enter as undefined symbol }
   plcsym(l, sp); { place new symbol }
   sp^.udf := true; { set symbol undefined }
   sp^.typ := gbludf; { attach to skeleton key } 
   if sp1 <> nil then begin 

      sp^.ref := sp^.ref + 1; { increment reference counter on undefined }
      sp := sp1; { set our symbol as the respelling }
      if not sp^.mis then { we haven't already respelled this symbol }
         error(emspell, false, l, sp^.lab^); { output misspelling message }
...
Process new symbol in scope

Performs new symbol semantics on the given symbol.

If the symbol found is a downreference, which is a symbol not at our level,
we also place a downreference marker in the current scope. The downreference
is a no-op when searching, and is removed when the level is removed. However,
if "define" encounters it, then it flags an error, since it means there was
already a reference within this region.
...
procedure newsym(view l:  string;  { label }
                      sp: symptr); { symbol pointer }

begin

   if sp^.ddf then begin { symbol is a delayed definition }

      error(efwdnptr, false, l); { inappropriate reference error }
      sp^.ddf := false { change status to normal }

   end else if sp^.hld then begin { symbol is in "holding" }

      error(eslfref, false, l); { self reference error }
      sp^.hld := false { change status to normal }

   end;
   sp^.ref := sp^.ref + 1; { increment reference counter }
   sp^.seq := curseq { set current sequence of reference }
...
procedure find(view l:  string;  { label }
               var  sp: symptr); { symbol }

begin

   sp := gblsym(l); { find global symbol }
   if sp = nil then begin { symbol not found }

      error(esymnf, false, l); { output symbol not found error }
      symudf(l, sp) { process symbol undefined }

   end else newsym(l, sp) { process as new scope symbol }
...
begin
   ...
   if chkovld(tp) then begin

      { If its an overload, all overloads are unified under one name. Go to the
        head entry }
      if tp^.t = tfunc then tp := tp^.fnch { link function head }
      else if tp^.t = tproc then tp := tp^.prch { link procedure head }
      else error(esflt37, true) { should have been one of those }
      ...
      for i := 1 to symmax do begin { traverse the symbols head }
         ...
         while p <> nil do begin { tranverse chain }
            ...
            { check symbol matches current scope, is not exportable, and has no
              references }
            if (p^.lvl = level) and (p^.ref = 0) and not p^.exp then
               error(esymnr, false, p^.lab^); { no references }
            nxtsym(p, mp, i) { next symbol }
...
begin
   ...
   tp^.classt := curcls; { set pointer to current class }
...
begin
   ...
   { if no bottom of update entry exists, set this as the start of the next
     update }
   if blkstk^.upd = nil then blkstk^.upd := tp;
   tp^.lvl := typlvl; { set level number }
   tp^.num := blkstk^.seq; { set sequential type number }
   blkstk^.seq := blkstk^.seq+1 { next number }
...
begin
   ...
   while blkstk^.typ <> nil do begin { while entries left in types list }

      tp := blkstk^.typ; { index top entry }
      blkstk^.typ := blkstk^.typ^.next; { gap top of list }
...
begin
   ...
   while tl <> nil do begin { while entries left in types list }
      ...
      tp^.next := blkstk^.typ; { insert into type list }
      blkstk^.typ := tp
...
var p:        typptr;  { type entry pointer }
    ...
    sp:       blkptr;  { type lists pointer }
    sm:       labl;    { block marker }

begin

   sp := blkstk; { index top of stack }
   m := 0; { set no match }
   if tp = nil then write('nil') { write marker for nil }
      ...
      while p <> nil do begin { traverse list }
         ...
         if p = tp then begin m := c; copy(sm, sp^.blk^) end;
         p := p^.next; { next entry }
         c := c + 1 { count entries }
...
begin
   ...
   case tp^.t of { type }
      ...
      tudf: begin write('undefined'); writeln end;           
      tnil: begin write('nil (pointer value)'); writeln end;
      tlab: begin 

         write('goto label, defined: ');
         write(tp^.ldef);
         write(' references: ');
         write(tp^.lref:1);
         write(' statement level: ');
         writeln(output, tp^.slvl:1)

      end;
      ticst: begin write('integer constant: '); if tp^.ival.s then write('-');
                   writeln(output, tp^.ival.v:1) end;
      tscst: begin 
      ...
      tccst: begin write('Character constant: '''); 
                   writeln(output, tp^.cval, '''') end;
      trcst: begin write('real constant: '); 
                   writeln(output, tp^.rval) end;
      tstcst: begin
      ...
      tstet: begin
         ...
         if tp^.stes.s then write('-');
         write(tp^.stes.v:1); 
         write(' ending value: ');
         if tp^.stee.s then write('-');
         write(tp^.stee.v);
         write(' head: ');
         listety(tp^.steh);
         writeln
      ...
      tsub: begin 
         ...
         write('subrange: ');
         if tp^.subl.s then write('-');
         write(tp^.subl.v:1);
         write('..');
         if tp^.subu.s then write('-');
         write(tp^.subu.v:1);
         write(' base: '); 
         listety(tp^.subt); 
         writeln 
      ...
      tfcas: begin
         ...
         write('variant record case, constant: ');
         wrtssint(tp^.fcss);
         if ssnequ(tp^.fcss, tp^.fcse) then begin
      ...
      tvar: begin 
         ...
         write('variable, base: '); 
         listety(tp^.vart); 
         writeln(' threat count: ', tp^.varr:1, ' ''for'' use count: ', 
                 tp^.varf:1, ' extern: ', tp^.vare:0, ' sub threat: ', 
                 tp^.varp:0, ' member: ', tp^.varm:0)
      ...
      tproc: begin 
         ...
         if tp^.prca then write(' asm ');
         if tp^.prcx then write(' collide ');
         if tp^.prcq then write(' del_fwd_ovl ');
         if tp^.prcg then write(' static ');
         if tp^.prcv then write(' virt ');
         if tp^.prcz <> nil then 
            begin write(' ovr link: '); listety(tp^.prcz) end;
         writeln 
      ...
      tfunc: begin 
         ...
         if tp^.fnce then write(' asm ');
         if tp^.fncx then write(' collide ');
         if tp^.fncq then write(' del_fwd_ovl ');
         if tp^.fncg then write(' static ');
         if tp^.fncv then write(' virt ');
         if tp^.fncz <> nil then 
            begin write(' ovr link: '); listety(tp^.fncz) end;
         writeln 
      ...
      tlinteger: writeln('linteger');
      tcardinal: writeln('cardinal');
      tlcardinal: writeln('lcardinal');
...
"type" section as started them, because they may be used inprocedures/functions
where their types are checked.
...
Also checks if the entry is a downreference alias. If this is the case, then
the alias was a "success" in that it didn't get overriden by another define
within the current block, so we transfer its reference counts down to the
other reference so that it will be correct for references.
...
begin
   ...
   while tp <> nil do begin { traverse list }
      ...
      if tp^.t = tddf then begin

         if not tp^.ddfd and not tp^.ddfe then begin

            { Entry was never defined, and no error has been flagged. If the
              head symbol has been redefined, the association between symbol
              and entry is broken. Therefore, we have kept a private copy of
              the symbol pointer, and get the symbol from that. }
            error(emfwddef, false, tp^.ddfs^.lab^); { missing forward 
                                                      definition }
            tp^.ddfe := true { set error was processed }

         end else if tp^.ddfr <> nil then { its a downreference }
            { add in new references }
            tp^.ddfr^.ref := tp^.ddfr^.ref+tp^.ddfs^.ref
   ...
   if uselvl = 0 then begin { not in a 'uses' file }
      ...
      while tp <> nil do begin { traverse list }
         ...
         if tp^.t = tvar then if tp^.vars = fshparm then begin
            ...
            perror(ehfudf, [], [], sp^.lab^); { undefined header file }
...
begin
   ...
   while tp <> nil do begin { traverse list }
      ...
      if tp^.t in [tlab, tvar, tproc, tfunc] then case tp^.t of { type }
            ...
            if s <> nil then begin { there is a symbol }
               ...
               if s^.ref <> 0 then begin 
                  ...
                  { if the reference count for the symbol is non-zero, we 
                    output an error. If it is zero, we don't output, because the
                    error message would be redundant with the unreferenced 
                    symbol check done elsewhere }
                  if fref then begin { reference checks on }
   
                     if not tp^.ldef then 
                        error(elabndf, false, s^.lab^) { no target }
                     else error(elabref, false, s^.lab^) { no 'goto' 
                                                           references }
               ...
               end else if not fref and not tp^.ldef then
                  { Reference count is zero, no reference checking is enabled,
                    and label was never targeted. Because this won't be flagged
                    as a reference error, we complain about no target. }
                  error(elabndf, false, s^.lab^) { no target }
         ...
         tvar: if (tp^.varr = 0) and fass and not tp^.vare and 
                  not (tp^.vart^.t = tsemaphore) and
                  not (tp^.vart^.t = texception) then begin

            { No threats to variable, assign checks are on, not external, not a
              global level variable, and not a semaphore or exception, which are self-initializing. }
            s := labtyp(tp); { find the symbol for the entry }
            if s <> nil then begin { symbol exists }

               if (s^.ref <> 0) and fref and (tp^.lvl <> 2) then
                  { we didn't already flag this undefined, and not global level }
                  error(evarass, false, s^.lab^) { variable assignment }
         ...
         tproc: if tp^.prcf and not tp^.prce then begin { no body defined }
            ...
            { check override, these entries have no head symbol }
            if (tp^.prcz <> nil) and not tp^.prcv then begin

               s := labtyp(tp^.prcz); { get label for virtual entry }
               if s <> nil then { symbol exists }
                  error(efwdorndf, false, s^.lab^) { forward never defined }

            end else begin { standard or overload }
               ...
               if s <> nil then begin { symbol exists }
                  ...
                  if chkovld(tp) then { overload } 
                     error(efwdondf, false, s^.lab^) { forward never defined }
                  else { normal }
                     error(efwdndf, false, s^.lab^) { forward never defined }
         ...
         tfunc: if tp^.fncf and not tp^.fnce then begin { no body defined }
            ...
            { check override, these entries have no head symbol }
            if (tp^.fncz <> nil) and not tp^.fncv then begin

               s := labtyp(tp^.fncz); { get label for virtual entry }
               if s <> nil then { symbol exists }
                  error(efwdorndf, false, s^.lab^) { forward never defined }
   ...
   if curprc <> nil then begin { there is a procedure/function active }
      ...
      if curprc^.t = tfunc then { it is a function }
         ...
         if (s <> nil) and ffncrs then 
            { symbol exists and check function result }
            error(enfncra, false, s^.lab^) { no function result }
...
Counts overrides in the given entry. Given any procedure or function type in
an override list, counts the current number of entries in that list. The head
type is linked, then the number of items in the override list is counted and
returned.

*******************************************************************************}

function ovrcnt(tp: typptr): integer;

var cnt: integer; { override counter }
    tp1: typptr;  { type holding }

begin

   { link head entry }
   if tp^.t = tproc then tp1 := tp^.prcu { link procedure head }
   else if tp^.t = tfunc then tp1 := tp^.fncu { link function head }
   else error(esflt47, true); { otherwise fault }
   if tp1 <> nil then tp := tp1; { set head if found }
   cnt := 0; { set initial count }
   while tp <> nil do begin { count entries }

      if tp^.t = tproc then tp := tp^.prcz { link procedure next override }
      else if tp^.t = tfunc then tp := tp^.fncz { link function next override }
      else error(esflt48, true); { otherwise fault }
      if tp <> nil then cnt := cnt+1 { count entry }

   end;

   ovrcnt := cnt { return result }

end;

{*******************************************************************************

Count overrides to class

Counts overrides in the given entry that exist in the given class. Given any 
procedure or function type in an override list, counts the current number of
entries in that list. The head type is linked, then the number of matching items
in the override list is counted and returned.

*******************************************************************************}

function ovrcntcls(tp: typptr;  { procedure/function entry }
                   cls: typptr) { class to match }
                   : integer;   { number of overriders }
...
begin
   ...
   while tp <> nil do begin { count entries }
      ...
      if tp <> nil then if tp^.classt = cls then cnt := cnt+1; { count entry }

   end;

   ovrcntcls := cnt { return result }

end;

{*******************************************************************************

Find forwarded override entry

Searches the override list for a pending forward. Returns that if found,
otherwise returns nil.

*******************************************************************************}

function ovrfwd(tp: typptr): typptr;

var tp1: typptr; { type holding }
    fp:  typptr; { found entry }
...
begin
   ...
   while tp <> nil do { count entries }
      if tp^.t = tproc then begin

         tp := tp^.prcz; { link procedure next override }
         if tp <> nil then if tp^.prcf then fp := tp { set forward found }

      end else if tp^.t = tfunc then begin

         tp := tp^.fncz; { link function next override }
         if tp <> nil then if tp^.fncf then fp := tp { set forward found }

      end else error(esflt48, true); { otherwise fault }

   ovrfwd := fp { return result }

end;

{*******************************************************************************

Push base classes on uses stack

Pushes all of the base classes of a given class, then the class itself, onto the
uses stack. Used to open the scope up for a class. The caller is responsible for
removing all of the classes from the stack.

The classes are pushed in order of oldest parent first. This is not absolutely
required, since class member names are mutually exclusive, but it is neater.

*******************************************************************************}

procedure pshbas(tp: typptr); { base class to push }

var mp:  modptr; { module pointer }
    mlp: mltptr; { module list pointer }

begin

   { fault check valid type }
   if not (tp^.t in [tclass, tatom, tthread]) then error(esflt49, true);
   case tp^.t of
   
      tclass: begin 
   
         if tp^.clsi <> nil then pshbas(tp^.clsi); { go depth }
         mp := tp^.clss { get symbols module }
   
      end;
      tatom: begin
   
         if tp^.atmi <> nil then pshbas(tp^.atmi); { go depth }
         mp := tp^.atms { get symbols module }
   
      end;
      tthread: begin
   
         if tp^.thdi <> nil then pshbas(tp^.thdi); { go depth }
         mp := tp^.thds { get symbols module }
   
      end
   
   end;
   { push inherited block onto uses list }
...
begin
   ...
   tp := baset(tp); { find actual type }
   if tp^.t = tscst then begin { string constant }

      f := max(tp^.sval^) = 1 { true if single character }

   end else if tp^.t = tarray then begin { array }

      if tp^.pack then { packed }
         if chart(tp^.arrt) then { base char }
            { check 1..1 array }
            f :=  (lbound(tp^.arri) = 1) and (ubound(tp^.arri) = 1)

   end else if tp^.t = tgarry then begin { general array }

      if tp^.pack then { packed }
         f := chart(tp^.gart) { base is char }

   end else f := (tp^.t = tchar) or (tp^.t = tccst); { char or char const }

   sinchr := f { return result }
...
begin
   ...
   if pa^.t = pb^.t then begin { procedure/function status is the same }
      ...
      while (ppa <> nil) and (ppb <> nil) do begin { compare parameters }
         ...
         if typcmp(ppa, ppb) or (sinchr(ppa) and sinchr(ppb)) or
            (realt(ppa) and intt(ppb)) or (intt(ppa) and realt(ppb)) then
            begin

            if (((ppa^.t in [tpar, tvpar, twpar]) and 
                 (ppb^.t in [tpar, tvpar, twpar]) and (ppa^.t <> ppb^.t)) or
                (realt(ppa) and intt(ppb)) or (intt(ppa) and realt(ppb))) and
               m then begin
               ...
               if pa^.t = tproc then pa^.prcx := true else pa^.fncx := true;
               if pb^.t = tproc then pb^.prcx := true else pb^.fncx := true

            end

         end else m := false; { set does not match }
...
begin
   ...
   if tp^.t = ticst then i := tp^.ival.v { integer constant }
      ...
      if max(tp^.sval^) = 1 then { single character }
         i := chr2ascii(tp^.sval^[1]) { load character }
      else error(esflt26, true) { must be single character }

   end else if tp^.t = tccst then i := chr2ascii(tp^.cval) { character constant }
...
Loads the constant sign from the given type entry. Will load any of an integer,
single character string, or enumerated type.
...
begin
   ...
   end else if tp^.t = tccst then s := false { character constant }
...
begin
   ...
   if tp^.t <> tscst then begin { check array type }
      ...
      if ((tp^.t <> tarray) and (tp^.t <> tgarry)) 
         ...
         if tp^.t = tarray then begin { standard array }

            tp1 := actt(tp^.arrt); { index base type }
            if not intt(tp^.arri) then m := false { not integer index }

         end else tp1 := actt(tp^.gart);
         if tp1^.t <> tchar then m := false { must be character }
            ...
            if (tp1^.subl.v <> 1) or tp1^.subl.s then 
               m := false { must be 1 based }
...
Checks if the given type is a class. This can be any of a class, atom or thread.

*******************************************************************************}

function classt(tp: typptr): boolean;

begin

   tp := baset(tp); { find the base type }

   classt := tp^.t in [tclass, tatom, tthread]
...
Checks if the given type is a reference.

*******************************************************************************}

function reft(tp: typptr): boolean;

begin

   tp := baset(tp); { find the base type }

   reft := tp^.t = treference
...
begin
   ...
   if tp^.t = tscst then len := max(tp^.sval^) { string constant }
      ...
      { find length }
      len := ssadd(tp^.subu.s, tp^.subu.v, not tp^.subl.s, tp^.subl.v)+1
...
Compares to classes for compatibility. Two classes are compatible if they have
a base class in common. This means that they have members in common, and that it
is conceivable that they might participate in a compare or assignment operation.
Note that this is not the same as as the "is" operator comparision, which checks
if "a is b" or that a contains all the members of b.

*******************************************************************************}

function clscmp(a, b: typptr): boolean;

var m: boolean; { match status }

{ link base class }

procedure bascls(var tp: typptr);
...
function matcls(a, b: typptr): boolean;

var m: boolean; { match status }

begin

   { walk b while comparing a }
   m := false; { set no match }
   while b <> nil do
      if a = b then begin m := true; b := nil end { found }
      else bascls(b); { link next class b }

   matcls := m { return result }
...
begin { clscmp }
   ...
   m := false; { set no match }
   if matcls(a, b) then m := true { match a to b }
   else if matcls(b, a) then m := true; { match b to a }

   clscmp := m { return match status }
...
{ match general array to standard array indexes, to any depth }

function garmat(tp1, tp2: typptr): boolean;

var tp: typptr; { temp }

begin

   { swap to make sure that general array is left }
   if tp2^.t = tgarry then begin 
   ...
   while (tp1^.t = tgarry) and ((tp2^.t = tgarry) or (tp2^.t = tarray)) do begin

      { advance left pointer to next index }
      tp1 := tp1^.gart;
      { advance right pointer to next index }
      if tp2^.t = tarray then tp2 := tp2^.arrt
      else if tp2^.t = tgarry then tp2 := tp2^.gart

   end;

   garmat := tp1 = tp2 { return match status }
   ...
   if (tp1 <> tp2) and (tp1^.t <> tudf) and (tp2^.t <> tudf) then begin 
      ...
      if strt(tp1) and strt(tp2) then begin { both are strings }
         ...
         if (tp1^.t = teset) or (tp2^.t = teset) then m := true
            ...
            { Base types must be equal now. All integers are also considered 
              equal. }
            if (baset(tp1) <> baset(tp2)) and
               not (intt(tp1) and intt(tp2)) then m := false

         end

      end else if (tp1^.t in [tarray, tgarry]) and 
                  (tp2^.t in [tarray, tgarry]) and not
                  ((tp1^.t = tarray) and (tp2^.t = tarray)) then begin 

         { both are arrays or general arrays, but not both arrays }
         if tp1^.t = tarray then tp1 := tp1^.arrt { get base types }
         else tp1 := tp1^.gart;
         if tp2^.t = tarray then tp2 := tp2^.arrt
         else tp2 := tp2^.gart;
         { match is if bases are equal, or if any number of general array 
           indecies match either a fixed or general array }
         m := (tp1 = tp2) or garmat(tp1, tp2) { base types must be equal now }

      end else if chart(tp1) and chart(tp2) then m := true { if both chars }
      else if ((tp1^.t = tptr) and (tp2^.t = tnil)) or
              ((tp1^.t = tnil) and (tp2^.t = tptr)) or
              ((tp1^.t = treference) and (tp2^.t = tnil)) or
              ((tp1^.t = tnil) and (tp2^.t = treference)) then
         m := true { one is pointer, and the other is 'nil' key }
      else if (tp1^.t = treference) and (tp2^.t = treference) then
         { If both are references, then they are compatible if any of their base
           classes are the same }
         m := clscmp(tp1^.reft, tp2^.reft)
      else if realt(tp1) and realt(tp2) then m := true { both are variants of real }
      else m := intt(tp1) and intt(tp2) { both are variants of integer }
...
Adds a new level to the block stack.

*******************************************************************************}

procedure pshblk(sp: symptr; tp: typptr);

var bp: blkptr; { stack list pointer }

begin

   if (blkfre <> nil) and frecir then begin { recover entry from free list }

      bp := blkfre; { index top entry }
      blkfre := blkfre^.next { gap list }
      
   end else new(bp); { get new list entry }
   bp^.next := blkstk; { link to top of stack }
   blkstk := bp; { place new top }
   bp^.typ := nil; { clear new list }
   bp^.lst := nil; { set no last entry }
   bp^.upd := nil; { set no update entry }
   bp^.blk := nil; { clear block id }
   if sp <> nil then bp^.blk := copy(sp^.lab^); { place block id }
   bp^.seq := 1; { set 1st sequence number }
   typlvl := typlvl+1; { add typing level }
   bp^.lvl := typlvl; { set level }
   bp^.marks := sp; { set mark symbol }
   bp^.mark := tp { set mark type }
...
var bp: blkptr; { type list entry pointer }

begin

   if blkstk = nil then error(esflt7, true); { fatal error on stack empty }   
   bp := blkstk; { index top entry }
   blkstk := blkstk^.next; { remove top entry }
   bp^.next := blkfre; { link into free list }
   blkfre := bp;
...
Dump contents of block stack

Prints all of the blocks on the block stack, from most recent nested to least
recent. A diagnostic.
...
var bp: blkptr; { pointer in block list }
...
begin
   ...
   while bp <> nil do begin { dump blocks }
      ...
      { print level and label }
      writeln(output, bp^.lvl:1, ' ', bp^.marks^.lab^);
...
begin
   ...
   else if intt(tp) then s := true { return integer negative }
...
begin
   ...
   else if intt(tp) then s := false { return integer unsigned }
   else if chart(tp) then s := false { return character unsigned }
   else if tp^.t = tboolean then s := false { return boolean unsigned }
   ...
   if tp^.t in [tvar, tfix, tpar, tvpar, twpar] then begin
      ...
      case tp^.t of
         ...
         tvar:  begin
            ...
            tp^.varr := tp^.varr + 1;
            { check in use by 'for' }
            if tp^.varf <> 0 then perror(eforviu, [], [], sp^.lab^); { variable in use }
...
procedure chktkmp(view l: string;   { label to check }
...
begin
   ...
   if nxttlk = cidentifier then begin { if the next is an id }
      ...
      sp := gblsym(l); { check symbol exists without qualident }
      if sp = nil then begin { symbol does not exist }

         sp := mgblsym(l); { check symbol is not a possible misspell of 
                          existing symbol }
         if sp = nil then begin { not found }
            ...
            if cort <> cundefined then begin { correct the spelling }
               ...
               perror(esymnf, [], [], l); { symbol not found }
...
Check system special procedure or function

Returns true if the indicated type is a special function or procedure.

*******************************************************************************}

function sysspc(tp: typptr): boolean;

var f: boolean; { output flag }

begin

   f := false; { set not special }
   if tp^.t = tproc then f := tp^.prcd <> pfnil
   else if tp^.t = tfunc then f := tp^.fncd <> pfnil;
   
   sysspc := f { return result }
...
begin
   ...
   if tp = nil then 
      { link is nil, output 0 for that }
      begin wrtnum(false, 0); wrtnum(false, 0) end
   else begin 
 
      wrtnum(false, tp^.lvl); { output level }
      wrtnum(false, tp^.num) { output entry number }
...
begin
   ...
   wrtstr(sp^.lab^) { output symbol }

end;

begin

   if fintopn and (blkstk^.upd <> nil) then begin 
      ...
      while tp <> nil do begin { traverse list }
         ...
         { otherwise perform output according to type }
            ...
            tscst:     begin { string constant }
               ...
               wrtstr(tp^.sval^) { write string }

            end;
            tccst:     begin { character constant }

              wrtcod(iccst); { output marker }
              wrtint(0); { extend to UCS-16 (Unicode) }
              wrtint(chr2ascii(tp^.cval)) { ouptut character value }
            ...
            tstet:     begin { constant set entry }
               ...
               wrtnum(tp^.stes.s, tp^.stes.v); { output starting value }
               wrtnum(tp^.stee.s, tp^.stee.v); { output ending value }
            ...
            tenme:     begin { enumerator constant entry }
               ...
               wrtnum(false, tp^.env) { output enumerated constant }
            ...
            tsub:      begin { subrange type }
               ...
               wrtnum(tp^.subl.s, tp^.subl.v); { output lower bound }
               wrtnum(tp^.subu.s, tp^.subu.v) { output upper bound }
            ...
            tfcas:     begin { record case constant type }
               ...
               wrtnum(tp^.fcss.s, tp^.fcss.v); { output case constant }
               wrtnum(tp^.fcse.s, tp^.fcse.v)
            ...
            tvar:      begin { variable type }
               ...
               wrtnum(false, tp^.varmd); { output module ordinal }
               wrtlnk(tp^.classt) { output class type link }
            ...
            tfix:      begin { fixed type }
               ...
               wrtnum(false, tp^.fixmd) { output module ordinal }
            ...
            tproc:     if not tp^.prcq then begin { procedure type }
               ...
               wrtnum(false, tp^.prcmd); { output module ordinal }
               wrtint(ord(tp^.prca)); { output assembly flag }
               wrtlnk(tp^.prch); { output overload list head link }
               wrtint(ord(tp^.prcg)); { output static flag }
               wrtint(ord(tp^.prcv)); { output virtual flag }
               wrtlnk(tp^.prcz); { output overrider link }
               wrtlnk(tp^.prcu); { output override list head link }
               wrtlnk(tp^.classt) { output class type link }
            ...
            tfunc:     if not tp^.fncq then begin { function type }
               ...
               wrtnum(false, tp^.fncmd); { output module ordinal }
               wrtint(ord(tp^.fnca)); { output assembly flag }
               wrtlnk(tp^.fnch); { output overload list head link }
               wrtint(ord(tp^.fncg)); { output static flag }
               wrtint(ord(tp^.fncv)); { output virtual flag }
               wrtlnk(tp^.fncz); { output overrider link }
               wrtlnk(tp^.fncu); { output override list head link }
               wrtlnk(tp^.classt) { output class type link }
            ...
            tlinteger:  wrtcod(ilint); { long integer type }
            tcardinal:  wrtcod(icard); { cardinal type }
            tlcardinal: wrtcod(ilcard); { long cardinal type }
            ...
            tsemaphore: wrtcod(isemaphore); { semaphore }
            tclass:     begin

               wrtcod(iclass); { class }
               wrtlnk(tp^.clsi) { output base type link }

            end;
            tatom:      begin

               wrtcod(iatom); { atom }
               wrtlnk(tp^.atmi) { output base type link }

            end;
            tthread:    begin

               wrtcod(ithread); { thread }
               wrtlnk(tp^.thdi) { output base type link }

            end;
            treference: begin

               wrtcod(ireference); { class reference }
               wrtlnk(tp^.reft) { output base type link }

            end;
            texception: wrtcod(iexception); { exception }

         end;
         tp := tp^.next { index next type entry }

      end;
      blkstk^.upd := nil { clear next update entry }
...
Downreference markers are ignored,
...
procedure wrtsyms(mp: mltptr);

var mp1: mltptr; { pointer for module list }
    i:   syminx; { index for symbol table }
    p:   symptr; { pointer for symbols }
...
begin
   ...
   if fintopn then begin { the intermediate file is open }
      ...
      wrtsyms(uselst); { output uses list }
      wrtsyms(joinlst) { output joins list }
