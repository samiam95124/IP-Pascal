...
procedure error(e: errcod); forward;
...
begin
   ...
   case e of { error }
      ...
      edupsym:  writeln('Duplicate symbol');
      ...
      esys:     writeln('System fault, notify software vendor');
...
begin
   ...
   if deftab[di] = p then deftab[di] := p^.next { gap from top }
      ...
      if dp^.next = nil then error(esys); { should not happen }
...
begin
   ...
   for i := 1 to lenp(s) do begin { process }
   ...
   copyp(s, buf) { copy back to result }
...
begin
   ...
   if (fp <> nil) and (frp <> nil) then begin { found match }
      ...
      if s+max(fp^.name^) <= lenp(mv) then { get all after }
         extractp(tmp, mv, s+max(fp^.name^), lenp(mv))
      else clears(tmp);
      copyp(tmp1, frp^.name^); { make copy of parameter }
      if quote then frcqut(tmp1); { if quoted, place forces for embedded quotes }
      lpar := lenp(tmp1); { get length for that }
      extractp(mv, mv, 1, s-1); { get all before }
      if quote then insertp(mv, '"', s); { place leading quote }
      insertp(mv, tmp1, s+ord(quote)); { add macro contents }
      { place trailing quote }
      if quote then insertp(mv, '"', s+lpar+1);
      insertp(mv, tmp, s+lpar+2*ord(quote)); { add all after }
...
begin
   ...
   if chkchr <> '(' then error(esys); { no parameters }
   ...
   if chkchr = ')' then begin { empty list }
      ...
      repeat { parse parameters }
         ...
         copysp(pp1^.name, par); { place as name }
...
begin
   ...
   if mp <> nil then begin { state matches }
      ...
      copyp(linsav, s); { save copy of line }
      copyp(mcont, mp^.sval^); { copy substitute line to temp }
      ...
      if macend <= lenp(s) then { get all after }
         extractp(linend, s, macend, lenp(s))
      else clears(linend);
      extractp(s, s, 1, i-1); { get all before }
      insertp(s, mcont, i); { add macro contents }
      insertp(s, linend, i+lenp(mcont)); { add all after }
...
begin
   ...
   if chkchr = '(' then begin { (id) }
      ...
      skpspc; { skip spaces }
      getlab;
      if lenp(lab) = 0 then error(eidexp);
      skpspc; { skip spaces }
   ...
   { set macro value }
   if val then copyp(mcont, '1L') else copyp(mcont, '0L');
   { expand macro }
   copyp(linsav, s); { save copy of line }
   if i <= lenp(s) then { get all after }
      extractp(linend, s, i, lenp(s))
   else clears(linend);
   extractp(s, s, 1, ps-1); { get all before }
   insertp(s, mcont, ps); { add macro contents }
   insertp(s, linend, ps+2); { add all after }
...
begin
   ...
   lrs := lenp(rs); { find length of replacement string }
   ps := i; { set position at "define" }
   { skip label }
   while chkchr in ['_', 'a'..'z', 'A'..'Z', '0'..'9'] do i := i+1;
   { expand macro }
   ...
   extractp(s, s, 1, ps-1); { get all before }
   insertp(s, rs, ps); { add macro contents }
   insertp(s, linend, ps+lrs); { add all after }
...
begin
   ...
   extractp(s, s, 1, ps-1); { get all before }
   insertp(s, '0L', ps); { place "0L" id replacement }
   insertp(s, linend, ps+2); { add all after }
   fi := ps+2 { set follow index }
...
var macn:    linbuf;    { macro name }
    ...
    inquote: boolean;   { in quoted sequence }
      ...
      if macn[1] in ['_', 'a'..'z', 'A'..'Z'] then begin
         ...
         if not (compcp(lmacn, macn) and (sp = lsp)) then begin
            ...
            if compcp(macn, 'defined') and ifp then 
               ...
               intsp(tmps, srclst^.linnum); { convert line number to string }
               prcrstr(s, sp, tmps, i) { place }

            end else if compcp(macn, '__FILE__') then begin

               copyp(tmps, '"'); { set starting quote }
               catp(tmps, srclst^.inpnam); { add filename }
               catp(tmps, '"'); { set ending quote }
               ...
               { date, note that this definition is not perfectly standard
                 in format }
               datesp(tmps1, local(time)); { find string for local date }
               copyp(tmps, '"'); { set starting quote }
               catp(tmps, tmps1); { add date }
               catp(tmps, '"'); { set ending quote }
               prcrstr(s, sp, tmps, i) { place }
               ...
               { time, note that this definition is not perfectly standard
                 in format }
               timesp(tmps1, local(time)); { find string for local date }
               copyp(tmps, '"'); { set starting quote }
               catp(tmps, tmps1); { add time }
               catp(tmps, '"'); { set ending quote }
               prcrstr(s, sp, tmps, i) { place }
         ...
         copyp(lmacn, macn); { set last macro processed }
         lsp := sp
...
begin
   ...
   while i < max(s) do begin { look for ids }
      ...
      if (s[i] in ['_', 'a'..'z', 'A'..'Z', '0'..'9']) and not inquote then
         begin
         ...
         else if s[i] = '\\' then i := i+1; { force, skip further character }
...
begin
   ...
   if not eof(srclst^.inpfil) then repeat { force lines }
      ...
      readsp(srclst^.inpfil, tmpin, ovf); { get next line }
      ...
      if fprtlin then begin { list incoming lines }
         ...
         write(srclst^.linnum:1, ': ');
         writesp(output, tmpin);
         writeln

      end;
      trigraph(tmpin); { replace trigraphs }
      il := lenp(tmpin); { get line length }
      ...
      { check overflows }
      if il+lenp(srclst^.lininp) > maxlin then error(einltl);
      { concatenate to result }
      insertp(srclst^.lininp, tmpin, lenp(srclst^.lininp)+1)
...
begin
   ...
   if srclst <> nil then begin { not at files end }
      ...
      repeat { read lines possibly containing directives }
         ...
         if not eof(srclst^.inpfil) then begin { not eof }
      ...
      if not eof(srclst^.inpfil) then begin
         ...
         copyp(cmpbuf, srclst^.lininp); { save a copy of the line }
...
begin
  ...
  repeat
     ...
     while ((schkchr <= ' ') or (schkchr = '#')) and not sendlin do
        sgetchr;
     if sendlin then getlin { get a new line }
...
This is the same as the external version, but does not perform a getlin on the
file.
...
begin
   ...
   copyp(srclst^.inpnam, n) { place filename }
...
begin
   ...
   if srclst = nil then error(esys); { stack underflow should not occur }
...
procedure cexpr(var r: integer);

procedure expr13(var r: integer);
...
begin
   ...
   if schkchr = '(' then begin
      ...
      sgetchr; { skip }
      cexpr(r); { parse constant subexpression }
      sskpspc; { skip spaces }
      if schkchr <> ')' then error(erpexp);
      sgetchr { skip }
         ...
         if schkchr = '0' then begin { handle octal and hex }
            ...
            if lcase(schkchr) = 'x' then begin { hexadecimal }
               ...
               while schkchr in ['0'..'9', 'a'..'f', 'A'..'F'] do begin
                  ...
                  if schkchr in ['0'..'9'] then r := r*10+ord(schkchr)-ord('0')
                  else r := r*10+ord(lcase(schkchr))-ord('a')+10;
                  sgetchr
      ...
      end else error(einvfac) { invalid factor }

   end

end;

procedure expr12(var r: integer);
...
begin
   ...
   end else expr13(r) { parse downlevel }

end;

procedure expr11(var r: integer);
...
begin
   ...
   expr12(r); { parse downlevel }
   fndopr(op, false); { find operator }
   while op in [optimes, opdiv, opmod] do begin

      { there is a operator }
      fndopr(op, true); { skip operator }      
      expr12(r1); { parse downlevel }
      case op of { operator }
...
procedure expr10(var r: integer);
...
begin
   ...
   expr11(r); { parse downlevel }
   fndopr(op, false); { find operator }
   while op in [opplus, opminus] do begin

      { there is a operator }
      fndopr(op, true); { skip operator }      
      expr11(r1); { parse downlevel }
...
begin
   ...
   expr10(r); { parse downlevel }
   fndopr(op, false); { find operator }
   if op in [opshl, opshr] then begin

      { there is a operator }
      fndopr(op, true); { skip operator }      
      expr10(r1); { parse downlevel }
...
begin
   ...
   expr9(r); { parse downlevel }
   fndopr(op, false); { find operator }
   while op in [opltn, oplequ, opgtn, opgequ] do begin

      { there is a operator }
      fndopr(op, true); { skip operator }      
      expr9(r1); { parse downlevel }
      case op of { operator }
...
begin
   ...
   expr8(r); { parse downlevel }
   fndopr(op, false); { find operator }
   while op in [opequ, opnequ] do begin

      { there is a operator }
      fndopr(op, true); { skip operator }      
      expr8(r1); { parse downlevel }
...
begin
   ...
   expr7(r); { parse downlevel }
   fndopr(op, false); { find operator }
   while op = opand do begin { there is a operator }

      fndopr(op, true); { skip operator }      
      expr7(r1); { parse downlevel }
...
begin
   ...
   expr6(r); { parse downlevel }
   fndopr(op, false); { find operator }
   while op = opxor do begin { there is a operator }

      fndopr(op, true); { skip operator }      
      expr6(r1); { parse downlevel }
...
begin
   ...
   expr5(r); { parse downlevel }
   fndopr(op, false); { find operator }
   while op = opor do begin { there is a operator }

      fndopr(op, true); { skip operator }      
      expr5(r1); { parse downlevel }
      r := r or r1; { calculate }
      fndopr(op, false) { find operator }
...
begin
   ...
   expr4(r); { parse downlevel }
   fndopr(op, false); { find operator }
   while op = opland do begin { there is a operator }

      fndopr(op, true); { skip operator }      
      expr4(r1); { parse downlevel }
...
begin
   ...
   expr3(r); { parse downlevel }
   fndopr(op, false); { find operator }
   while op = oplor do begin { there is a operator }

      fndopr(op, true); { skip operator }      
      expr3(r1); { parse downlevel }
   ...
   repeat { look for termination }
      ...
      if not eof(srclst^.inpfil) then begin
         ...
         if hasdir then begin { contains a directive }
            ...
            sskpspcl; { skip leading spaces }
            if schkchr <> '#' then error(esys); { should be '#' }
            sgetchr; { skip '#' }
            getlab(dirn); { get label }
            if lenp(dirn) = 0 then error(edirmis); { no directive label present }
...
begin
   ...
   if compcp(dirn, 'include') then begin
      ...
      if lenp(macn) = 0 then error(emacexp); { no macro name }
      ...
      if dp = nil then begin { no previous definition }
            ...
            repeat { parse parameters }
               ...
               if lenp(parn) = 0 then begin
                 ...
                 c := schkchr { set termination character }
                  ...
                  copysp(pp^.name, parn); { place the name }
         ...
         copysp(dp^.sval, defbuf) { place definition string }
         ...
         if schkchr = '(' then begin
            ...
            repeat { parse parameters }
               ...
               getlab(parn); { get parameter id }
               if lenp(parn) = 0 then error(eparnf); { missing parameter }
   ...
   end else if lenp(dirn) > 0 then error(edirnf) { no directive found }
...
begin
   ...
   if fprtdef then begin { defines are to be printed }
      ...
      writeln('Definitions:');
      writeln;
      for di := 1 to maxdef do begin { traverse definitions table }
