...
var wc:         sc_wndclassa; { windows class structure }
    ...
    v:          integer;
    task1id:    integer; { dummy thread id (unused) }
    task1win:   integer;

{******************************************************************************
...
;prtstr('Main thread 1 gets message\cr\lf');
   if imsg = sc_wm_create then begin
      ...
      prtstr('Main thread 1 gets user message $400\cr\lf');
      r := 0
 
   end else if imsg = $401 then begin

      prtstr('Main thread 1 gets user message $401\cr\lf');
      r := 0
 
   end else if imsg = sc_wm_size then begin

      prtstr('Main thread 1 gets WM_SIZE message\cr\lf');
      r := 0
...
;prtstr('wndprocthread: '); prtmsgu(hwnd, imsg, wparam, lparam);
   if imsg = sc_wm_create then begin
      ...
      prtstr('Task1: WM_SIZE\cr\lf');
      b := sc_postmessage(winhan, sc_wm_size, wparam, lparam);
...
if false then begin
   ...
   { present the window }
   b := sc_showwindow(task1win, sc_sw_showdefault);
   { send first paint message }
   b := sc_updatewindow(task1win);
...
if true then begin
   ...
   { create subthread to handle window }
   r := sc_createthread_nn(0, task1, 0, task1id);
   ...
   wc.background := 0;
   wc.menuname   := nil;
   wc.classname  := str('dummy');
   { register that class }
   b := sc_registerclass(wc);
   { create the window }
   v := 2; { construct sc_hwnd_message, $fffffffd }
   v := not v;
   winhan := sc_createwindow('dummy', '', 0, 0, 0, 0, 0,
                              v {sc_hwnd_message}, 0, sc_getmodulehandle_n);
   { message handling loop }
   while sc_getmessage(msg, 0, 0, 0) <> 0 do begin { not a quit message }

{      prtstr('Main passes queue message\cr\lf');}
      b := sc_translatemessage(msg); { translate keyboard events }
      r := sc_dispatchmessage(msg);
{      if msg.message = sc_wm_size then 
         prtstr('Main passes wm_size message\cr\lf');}
