...
begin
   ...
   if fn > cmdfil then begin { file is not a special file }
      ...
      with opnfil[fn]^ do begin { process file }
         ...
         if fn > cmdfil then { not a special file }
            if sc_close(hdl) < 0 then error(eclsfil); { bad file close }
...
begin
   ...
   with opnfil[fn]^ do begin { process file }
      ...
      if full then begin
         ...
         if sc_lseek(hdl, -1, 1) < 0 then
            error(epos); { file position error }
         full := false { clear buffer }

      end;
      { execute write }
      if sc_write(hdl, ba) < 0 then error(ewrite) { file write error }
...
begin
   ...
   with opnfil[fn]^ do begin { process file }
      ...
      { position file }
      if sc_lseek(hdl, p-1, 0) < 0 then error(epos); { file position error }
...
begin
   ...
   with opnfil[fn]^ do begin { process file }
      ...
      if cur < 0 then error(esize); { file size error }
...
begin
   ...
   dispose(fs); { release buffer }
   if r < 0 then error(edel) { delete error }
...
begin
   ...
   dispose(ss);
   if r < 0 then error(echg); { change error }
...
begin
   ...
   { open existing file in read mode }
   rv := sc_open(fs^, sc_o_rdonly, 0);
   dispose(fs); { release buffer }
   if rv >= 0 then { found it, close that file }
      if sc_close(rv) <> 0 then error(eclsfil); { bad file close }
