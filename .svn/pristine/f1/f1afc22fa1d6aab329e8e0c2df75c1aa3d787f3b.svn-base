...
{         if chncnt[2] < mccnt then 
   ...
   for i := 1 to mmax do begin { output result table }
      ...
      if ressym[i].reslab[1] <> ' ' then begin { not empty }
         ...
         write(outfile, '   copysp(restbl[', i:3, '].lab,  ''');
         for x := 1 to labmax do
            if ressym[i].reslab[x] <> ' ' then
               write(outfile, ressym[i].reslab[x]);
         write(outfile, ''');');
         if ressym[i].reschn <> 0 then { output chaining spec }
            write(outfile, ' restbl[', i:3, '].chn := ',
                          ressym[i].reschn:3, ';');
         writeln(outfile); { terminate line }
         write(outfile, '   restbl[', i:3, '].tolk := c');
         for x := 1 to labmax do if ressym[i].reslab[x] <> ' ' then
            write(outfile, ressym[i].reslab[x]);
         write(outfile, ';');
         writeln(outfile) { terminate line }
