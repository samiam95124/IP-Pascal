...
begin
   ...
   copy(nxtlab, outb) { place number as parsed }
...
begin
   ...
   copy(nxtlab, hold); { set up label for errors }
   search; { search triple characters }
   if (nxttlk = cundefined) or ((nxttlk = clnct) and not fcppcmt) then begin

      { search double characters }
...
begin
   ...
   restbl[  1].lab := copy('goto');
   restbl[  1].tolk := cgoto;
   restbl[  2].lab := copy('const');
   restbl[  2].tolk := cconst;
   restbl[  3].lab := copy('int');
   restbl[  3].tolk := cint;
   restbl[  4].lab := copy('default'); restbl[  4].chn :=   9;
   restbl[  4].tolk := cdefault;
   restbl[  5].lab := copy('static');
   restbl[  5].tolk := cstatic;
   restbl[  6].lab := copy('for');
   restbl[  6].tolk := cfor;
   restbl[  7].lab := copy('register');
   restbl[  7].tolk := cregister;
   restbl[  8].lab := copy('break');
   restbl[  8].tolk := cbreak;
   restbl[  9].lab := copy('union');
   restbl[  9].tolk := cunion;
   restbl[ 10].lab := copy('else'); restbl[ 10].chn :=   3;
   restbl[ 10].tolk := celse;
   restbl[ 11].lab := copy('short');
   restbl[ 11].tolk := cshort;
   restbl[ 12].lab := copy('void');
   restbl[ 12].tolk := cvoid;
   restbl[ 13].lab := copy('sizeof');
   restbl[ 13].tolk := csizeof;
   restbl[ 15].lab := copy('switch');
   restbl[ 15].tolk := cswitch;
   restbl[ 16].lab := copy('typedef');
   restbl[ 16].tolk := ctypedef;
   restbl[ 17].lab := copy('long');
   restbl[ 17].tolk := clong;
   restbl[ 19].lab := copy('extern'); restbl[ 19].chn :=  12;
   restbl[ 19].tolk := cextern;
   restbl[ 20].lab := copy('if');
   restbl[ 20].tolk := cif;
   restbl[ 22].lab := copy('enum');
   restbl[ 22].tolk := cenum;
   restbl[ 24].lab := copy('do');
   restbl[ 24].tolk := cdo;
   restbl[ 25].lab := copy('float');
   restbl[ 25].tolk := cfloat;
   restbl[ 26].lab := copy('auto'); restbl[ 26].chn :=   1;
   restbl[ 26].tolk := cauto;
   restbl[ 28].lab := copy('while');
   restbl[ 28].tolk := cwhile;
   restbl[ 29].lab := copy('return');
   restbl[ 29].tolk := creturn;
   restbl[ 30].lab := copy('unsigned');
   restbl[ 30].tolk := cunsigned;
   restbl[ 31].lab := copy('signed');
   restbl[ 31].tolk := csigned;
   restbl[ 32].lab := copy('double');
   restbl[ 32].tolk := cdouble;
   restbl[ 33].lab := copy('volatile');
   restbl[ 33].tolk := cvolatile;
   restbl[ 34].lab := copy('struct');
   restbl[ 34].tolk := cstruct;
   restbl[ 37].lab := copy('case');
   restbl[ 37].tolk := ccase;
   restbl[ 38].lab := copy('continue'); restbl[ 38].chn :=   7;
   restbl[ 38].tolk := ccontinue;
   restbl[ 39].lab := copy('char');
   ...
   deftbl[cint] := copy('int');
   deftbl[cdefault] := copy('default');
   deftbl[cstatic] := copy('static');
   deftbl[cfor] := copy('for');
   deftbl[cregister] := copy('register');
   deftbl[cbreak] := copy('break');
   deftbl[cunion] := copy('union');
   deftbl[celse] := copy('else');
   deftbl[cshort] := copy('short');
   deftbl[cvoid] := copy('void');
   deftbl[csizeof] := copy('sizeof');
   deftbl[cswitch] := copy('switch');
   deftbl[ctypedef] := copy('typedef');
   deftbl[clong] := copy('long');
   deftbl[cextern] := copy('extern');
   deftbl[cif] := copy('if');
   deftbl[cenum] := copy('enum');
   deftbl[cdo] := copy('do');
   deftbl[cfloat] := copy('float');
   deftbl[cauto] := copy('auto');
   deftbl[cwhile] := copy('while');
   deftbl[creturn] := copy('return');
   deftbl[cunsigned] := copy('unsigned');
   deftbl[csigned] := copy('signed');
   deftbl[cdouble] := copy('double');
   deftbl[cvolatile] := copy('volatile');
   deftbl[cstruct] := copy('struct');
   deftbl[ccase] := copy('case');
   deftbl[ccontinue] := copy('continue');
   deftbl[cchar] := copy('char');
